...
UINT32 cps3_flash_read(flash_chip * chip, UINT32 addr)
{
	...
	case FM_READID:
		...
		switch (addr & 0x7fffff) {
			case 0:	return 0x04040404;
			case 4: return 0xadadadad;
...
static INT32 Cps3Reset()
{
	...
	if(!BurnDrvGetHardwareCode() & HARDWARE_CAPCOM_CPS3_NO_CD){
		...
		// fast boot
		if (cps3_isSpecial) {
			Sh2Reset( *(UINT32 *)(RomGame), *(UINT32 *)(RomGame + 4) );
			Sh2SetVBR(0x06000000);
		} else {
			Sh2Reset( *(UINT32 *)(RomGame_D), *(UINT32 *)(RomGame_D + 4) );
			Sh2SetVBR(0x06000000);
		}
	}
...
static void DrvDraw()
{
	...
	for (INT32 i= 0; i< 2048; i += 4)
	{
		...
		INT32 ypos		= (RamSpr[i+1]&0x000003ff);
		...
		INT32 gscrolly		= (RamVReg[gscroll]&0x000003ff);
