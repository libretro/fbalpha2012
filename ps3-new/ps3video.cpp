/******************************************************************************* 
 * ps3video.cpp - FBA Next Slim PS3
 *
...
static void CalculateViewports()
{
	...
	else if ( (int)(device_aspect*1000) > (int)(m_ratio * 1000) )
	{
		...
		m_viewport_x_temp = (int)(temp_width * (0.5 - delta));
		m_viewport_y_temp = 0;
	...
	else if ( (int)(device_aspect*1000) < (int)(m_ratio * 1000) )
	{
		...
		m_viewport_x_temp = 0;
		m_viewport_y_temp = (int)(temp_height * (0.5 - delta));
...
void ps3graphics_psgl_init_device(uint32_t resolutionId, uint16_t pal60Hz, uint16_t tripleBuffering)
{
...
static int32_t ps3graphics_psgl_init(bool scaleEnable, uint32_t scaleFactor)
{
	...
	glBufferData(GL_TEXTURE_REFERENCE_BUFFER_SCE, 1024*1024, NULL, GL_STREAM_DRAW);

	glGenTextures(1, &tex);
	glBindTexture(GL_TEXTURE_2D, tex);

	glTextureReferenceSCE(GL_TEXTURE_2D, 1, SCREEN_RENDER_TEXTURE_WIDTH, SCREEN_RENDER_TEXTURE_HEIGHT, 0, SCREEN_RENDER_PIXEL_FORMAT, SCREEN_RENDER_TEXTURE_PITCH, 0);
...
void ps3graphics_set_resolution()
{
	cellVideoOutGetState(CELL_VIDEO_OUT_PRIMARY, 0, &m_stored_video_state);
}

void ps3graphics_init(bool scaleEnabled, uint32_t scaleFactor)
{
...
void ps3graphics_new(uint32_t resolution, uint32_t aspect, bool smooth, bool smooth2, const char * shader, const char* shader2, const char * menu_shader, bool overscan, float overscan_amount, bool pal60Hz, bool vsync, bool tripleBuffering, uint32_t viewport_x, uint32_t viewport_y, uint32_t viewport_width, uint32_t viewport_height, uint32_t scale_enabled, uint32_t scale_factor)
{
	psgl_device = NULL;
	psgl_context = NULL;
	gl_width = 0;
...
void ps3graphics_draw(int width, int height, uint8_t * screen, uint32_t drv_flags)
{
	...
	if (fbo_enable)
	{
		...
		cgGLSetParameter2f(_cgpTextureSize[0], width, height);
		cgGLSetParameter2f(_cgpOutputSize[0], fbo_vp_width, fbo_vp_height);
		cgGLSetParameter2f(_cgp_vertex_VideoSize[0], width, height);
		cgGLSetParameter2f(_cgp_vertex_TextureSize[0], width, height);
		cgGLSetParameter2f(_cgp_vertex_OutputSize[0], fbo_vp_width, fbo_vp_height);

		_cgp_timer[0] = cgGetNamedParameter(_fragmentProgram[0], "IN.frame_count");
		_cgp_vertex_timer[0] = cgGetNamedParameter(_vertexProgram[0], "IN.frame_count");
		cgGLSetParameter1f(_cgp_timer[0], frame_count);
		cgGLSetParameter1f(_cgp_vertex_timer[0], frame_count);
		...
		cgGLSetStateMatrixParameter(_cgpModelViewProj[1], CG_GL_MODELVIEW_PROJECTION_MATRIX, CG_GL_MATRIX_IDENTITY);
		cgGLSetParameter2f(_cgpVideoSize[1], fbo_vp_width, fbo_vp_height);
		cgGLSetParameter2f(_cgpTextureSize[1], fbo_width, fbo_height);
		cgGLSetParameter2f(_cgpOutputSize[1], _cgViewWidth, _cgViewHeight);
		cgGLSetParameter2f(_cgp_vertex_VideoSize[1], fbo_vp_width, fbo_vp_height);
		cgGLSetParameter2f(_cgp_vertex_TextureSize[1], fbo_width, fbo_height);
		cgGLSetParameter2f(_cgp_vertex_OutputSize[1], _cgViewWidth, _cgViewHeight);

		_cgp_timer[1] = cgGetNamedParameter(_fragmentProgram[1], "IN.frame_count");
		_cgp_vertex_timer[1] = cgGetNamedParameter(_vertexProgram[1], "IN.frame_count");
		cgGLSetParameter1f(_cgp_timer[1], frame_count);
		cgGLSetParameter1f(_cgp_vertex_timer[1], frame_count);
	...
	else
	{
		...
		cgGLSetParameter2f(_cgpVideoSize[0], width, height);
		cgGLSetParameter2f(_cgpTextureSize[0], width, height);
		cgGLSetParameter2f(_cgpOutputSize[0], _cgViewWidth, _cgViewHeight);
		cgGLSetParameter2f(_cgp_vertex_VideoSize[0], width, height);
		cgGLSetParameter2f(_cgp_vertex_TextureSize[0], width, height);
		cgGLSetParameter2f(_cgp_vertex_OutputSize[0], _cgViewWidth, _cgViewHeight);
...
void ps3graphics_switch_resolution(uint32_t resId, uint16_t pal60Hz, uint16_t tripleBuffering, uint32_t scaleEnabled, uint32_t scaleFactor)
{
	...
	ps3graphics_load_fragment_shader(DEFAULT_MENU_SHADER_FILE_, 2);
}
