...
unsigned char __fastcall Fantzn2xReadByte(unsigned int a)
{
	switch (a) {
		case 0xc41001: {
			return 0xff - System16Input[0];
...
unsigned short __fastcall Fantzn2xReadWord(unsigned int a)
{
#if 1 && defined FBA_DEBUG
...
void __fastcall Fantzn2xWriteWord(unsigned int a, unsigned short d)
{
	if (a >= 0x400000 && a <= 0x40ffff) {
		System16BTileWordWrite(a - 0x400000, d);
		return;
...
void Blox16bMap68K()
{
	...
	SekMapMemory(System16PaletteRam    , 0x840000, 0x840fff, SM_RAM);
	SekMapMemory(System16Ram           , 0xffc000, 0xffffff, SM_RAM);
	SekSetReadWordHandler(0, Fantzn2xReadWord);
	SekSetWriteWordHandler(0, Fantzn2xWriteWord);
	SekSetReadByteHandler(0, Fantzn2xReadByte);
	SekSetWriteByteHandler(0, Fantzn2xWriteByte);
	SekClose();
}
