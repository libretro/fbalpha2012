		...
		UINT32 key1, UINT32 key2, UINT32 key3, UINT32 key4)
{
	...
	UINT8 l = BITSWAP08(val, bitsB[7],bitsB[6],bitsB[5],bitsB[4],bitsB[3],bitsB[2],bitsB[1],bitsB[0]);
	UINT8 r = BITSWAP08(val, bitsA[7],bitsA[6],bitsA[5],bitsA[4],bitsA[3],bitsA[2],bitsA[1],bitsA[0]);
...
#ifdef SN_TARGET_PS3
//Total loop unroll and avoided shift by variable - which is microcoded on PPU
      //0
		if (inputs[0] != -1)
			res |= BIT(val, inputs[0]) << 0;
      //1
		if (inputs[1] != -1)
			res |= BIT(val, inputs[1]) << 1;
      //2
		if (inputs[2] != -1)
			res |= BIT(val, inputs[2]) << 2;
      //3
		if (inputs[3] != -1)
			res |= BIT(val, inputs[3]) << 3;
      //4
		if (inputs[4] != -1)
			res |= BIT(val, inputs[4]) << 4;
      //5
		if (inputs[5] != -1)
			res |= BIT(val, inputs[5]) << 5;
#else
	for (i = 0; i < 6; ++i)
	{
		if (inputs[i] != -1)
...
static const struct game_keys keys_table[] =
{
	...
	{ "jyangoku", { 0x6ca42ae6,0x92f63f59 }, 0x080000 },	// 0C80 3652 1573  cmpi.l  #$36521573,D0
	...
	// eeprom hack games for kaillera
	{ "avsp3p",   { 0x15208f79,0x4ade6cb3 }, 0x100000 },
	{ "avspu3p",  { 0xb4f61089,0xccf75a23 }, 0x100000 },
	{ "avspj3p",  { 0xe9dcb8fa,0x51372064 }, 0x100000 },
	{ "avspa3p",  { 0xc168f3bd,0x2e4a5970 }, 0x100000 },
	{ "batcir4p", { 0xd195e597,0x3cbce2b5 }, 0x200000 },
	{ "btcirj4p", { 0x00ff4dd8,0x000008e8 }, 0x200000 },
	{ "ddsom4p",  { 0x87889abc,0xd81f5f63 }, 0x100000 },
	{ "ddsomr4p", { 0x87889abc,0xd81f5f63 }, 0x100000 },
	{ "ddsomu4p", { 0x489f0526,0x1bcd3e7a }, 0x100000 },
	{ "ddsmur4p", { 0x489f0526,0x1bcd3e7a }, 0x100000 },
	{ "ddsomj4p", { 0xae92fa94,0x315a9045 }, 0x100000 },
	{ "ddsmjr4p", { 0xae92fa94,0x315a9045 }, 0x100000 },
	{ "ddsoma4p", { 0x8719abcd,0xef028345 }, 0x100000 },

	// 4p hacks
	{ "xmvsf4p",  { 0xbdcf8519,0x3fb2acea }, 0x100000 },
	{ "xmvsfr4p", { 0xbdcf8519,0x3fb2acea }, 0x100000 },
	{ "xmvsfu4p", { 0x4fcb03d2,0xf8653bc1 }, 0x100000 },
	{ "xmvsfj4p", { 0x38df93bc,0x210373ac }, 0x100000 },
	{ "xmvsfa4p", { 0x7438fc3e,0x19abed90 }, 0x100000 },
	{ "mshvsf4p", { 0x1384ae60,0x9cd725bf }, 0x100000 },
	{ "mshvsfu4p", { 0xa36d4971,0xcef51b28 }, 0x100000 },
	{ "mshvsfj4p", { 0x5dc391f8,0xa627e0b4 }, 0x100000 },
	{ "mshvsfa4p", { 0x52e3fa61,0x0c497bd8 }, 0x100000 },
	{ "mvsc4p",   { 0x48025ade,0x1c697b27 }, 0x100000 },
	{ "mvscu4p",  { 0x692dc41b,0x7ef1c805 }, 0x100000 },
	{ "mvscj4p",  { 0xafc16138,0x97123eb0 }, 0x100000 },
	{ "mvsca4p",  { 0xf248aec6,0x7905cd17 }, 0x100000 },

	{ 0 }	// end of table
};


int bsavedecryptedxor = 0;
int bsavedecryptedprom = 0;

const struct game_keys *k = NULL;
...
// ==> save decrypted prom, added by regret
void save_dec_prom()
{
	char* pRomName = NULL;
	char szName[25];
	...
	{
		BurnUpdateProgress(0.0, szSavingMessage, 0);

		// make decrypted prom name
		BurnDrvGetRomName(&pRomName, i, 0);
		strcpy(szTmp, pRomName);
		int nPos = strlen(szTmp);
		while (szTmp[nPos] != '.') nPos--;
		szTmp[nPos] = '\0';
		sprintf(szName, "%s", szTmp);
		sprintf(szName + strlen(szName), "d%s", pRomName + nPos);

		fp = fopen(szName, "wb");
		if (fp)
		{
			fwrite(CpsCode + nSize * i, nSize, 1, fp);
			fclose(fp);
		}
	}
}
...
void cps2_decryption()
{
	cps2_decrypt(k->keys, nCpsCodeLen);

	if (bsavedecryptedxor)
	{
		save_xor();
	}
	if (bsavedecryptedprom)
	{
		save_dec_prom();
	}
}
