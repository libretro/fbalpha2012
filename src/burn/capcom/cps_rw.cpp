...
//			bprintf(PRINT_NORMAL, _T("  - port 0x%02X (%3i)\n"), ia & 255, SekCurrentScanline());
		...
		if (ia == 0x009) {
			d = (unsigned char)~Inp009;
			return d;
		}
		if (ia == 0x01b) {
			d = (unsigned char)~Inp01b;
			return d;
		}
		if (ia == 0x029) {
...
//	if (d) bprintf(PRINT_NORMAL, _T("Write Port %x, %x\n"), ia, d);
	...
	if ((Cps & 1) && Cps1Qs == 0) {
		// CPS1 sound code
		if (ia == 0x181 || (kludge == 6 && ia == 0x006) || (Port6SoundWrite && (ia == 0x006 || ia == 0x007))) {
...
{
	CpsZRamC0[0x00] = sound_code >> 8;
	CpsZRamC0[0x01] = sound_code & 0xff;
