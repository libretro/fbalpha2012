...
static int CaveSpriteBuffer_NoZoom()
{
	...
	for (int i = 0, z = 0; i < 0x0400; i++, pSprite += 8) {
...
#else
		x = ( swapWord(pSprite[2]) + CaveSpriteVisibleXOffset) & 0x03FF;
#endif
		if (x >= 320) {
			if (x + xs <= 0x0400) {
				continue;
		...
		pBuffer->address =  swapWord(pSprite[1]) | ((word & 3) << 16);
...
static int CaveSpriteBuffer_ZoomA()
{
	...
	for (int i = 0, z = 0; i < 0x0400; i++, pSprite += 8) {
		...
		word =  swapWord(pSprite[2]);

		nPriority = (word >> 4) & 0x03;

		x = (( swapWord(pSprite[0]) >> 6) + CaveSpriteVisibleXOffset) & 0x03FF;
#if 0
		y = ((pSprite[1] >> 6) + nCaveExtraYOffset) & 0x03FF;
#else
		y = ( swapWord(pSprite[1]) >> 6) & 0x03FF;
#endif

		if ( swapWord(pSprite[4]) <= 0x0100 &&  swapWord(pSprite[5]) <= 0x0100) {
			if (x >= 320) {
				if (x + xs <= 0x0400) {
					continue;
				}
		...
		pBuffer->xzoom =  swapWord(pSprite[4]);
		pBuffer->yzoom =  swapWord(pSprite[5]);
		...
		pBuffer->address =  swapWord(pSprite[3]) | ((word & 3) << 16);
...
#else
		x = ( swapWord(pSprite[0]) + CaveSpriteVisibleXOffset) & 0x03FF;
#endif
#if 0
		y = (pSprite[1] + nCaveExtraYOffset) & 0x03FF;
#else
		y =  swapWord(pSprite[1]) & 0x03FF;
#endif
...
static int CaveSpriteBuffer_PowerInstinct()
{
	...
	for (int i = 0, z = 0; i < 0x0400; i++, pSprite += 8) {
		...
		x = ( swapWord(pSprite[2]) + nCaveExtraXOffset) & 0x03FF;
		if (x >= 320) {
			if (x + xs <= 0x0400) {
				continue;
			}
		...
		y = ( swapWord(pSprite[3]) + nCaveExtraYOffset) & 0x03FF;
		if (y >= 240) {
			if (y + ys <= 0x0400) {
				continue;
			}
