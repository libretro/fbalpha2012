...
static void CaveQueue8x8Layer_Normal(int nLayer)
{
	...
	for (y = 0; y < (31 << 8); y += (1 << 8)) {
		...
		for (x = mx; x >= 0; x -= (1 << 2)) {
			...
			nTileNumber = swapWord(*((unsigned short*)(pTileRAM + 0x4000 + nTileRow + nTileColumn))) << 16;
			nTileNumber |= swapWord(*((unsigned short*)(pTileRAM + 0x4002 + nTileRow + nTileColumn)));
...
static void Cave8x8Layer_RowScroll(int nLayer, unsigned int nPriority)
{
	...
	if (nPriority == 0) {
		...
		for (y = 0; y < nCaveYSize; y++) {
			rowscroll[y] = swapWord( *((unsigned short*)(pTileRAM + 0x1000 + (((dy + y) & 0x01FF) << 2)))) + bx;
		}
	}
...
static void Cave16x16Layer_RowSelect(int nLayer, unsigned int nPriority)
{
	...
	if (nPriority == 0) {
		...
		if (CaveTileReg[nLayer][0] & 0x4000) {	// Row-scroll mode
			for (y = 0; y < nCaveYSize; y++) {
				rowselect[y] = swapWord(*((unsigned short*)(pTileRAM + 0x1002 + (((cy + y) & 0x01FF) << 2))));
				rowscroll[y] = swapWord(*((unsigned short*)(pTileRAM + 0x1000 + (((cy + y) & 0x01FF) << 2)))) + bx;
			}
		} else {
			for (y = 0; y < nCaveYSize; y++) {
				rowselect[y] = swapWord(*((unsigned short*)(pTileRAM + 0x1002 + (((cy + y) & 0x01FF) << 2))));
