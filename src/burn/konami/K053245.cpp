...
#define NUM_SPRITES 128
	...
	for (pri_code = 0; pri_code < NUM_SPRITES; pri_code++)
	{
		...
		size = ( swapWord(sprbuf[offs]) & 0x0f00) >> 8;
        ...
        */
		zoomy =  swapWord(sprbuf[offs+4]);
		if (zoomy > 0x2000) continue;
		if (zoomy) zoomy = (0x400000+zoomy/2) / zoomy;
		else zoomy = 2 * 0x400000;
		if (( swapWord(sprbuf[offs]) & 0x4000) == 0)
		{
			zoomx =  swapWord(sprbuf[offs+5]);
...
//          else zoomx = zoomy; /* workaround for TMNT2 */
		...
		ox =  swapWord(sprbuf[offs+3]) + spriteoffsX;
		oy =  swapWord(sprbuf[offs+2]);

		ox += K053245_dx[chip];
		oy += K053245_dy[chip];

		flipx =  swapWord(sprbuf[offs]) & 0x1000;
		flipy =  swapWord(sprbuf[offs]) & 0x2000;
		mirrorx =  swapWord(sprbuf[offs+6]) & 0x0100;
		if (mirrorx) flipx = 0; // documented and confirmed
		mirrory =  swapWord(sprbuf[offs+6]) & 0x0200;
		shadow =  swapWord(sprbuf[offs+6]) & 0x0080;
