...
static int DrvDraw()
{
	if (DrvRecalc) {
		...
		for (int i = 0; i < 0x1000; i++) {
			...
			DrvPalette[i+1] = Palette[DrvColTable[i+1]];
			DrvPalette[i+2] = Palette[DrvColTable[i+2]];
			DrvPalette[i+3] = Palette[DrvColTable[i+3]];
#else
...
struct BurnDriver BurnDrvContrab = {
	...
	NULL, NULL, NULL, NULL,
	BDF_GAME_WORKING | BDF_CLONE | BDF_BOOTLEG | BDF_ORIENTATION_VERTICAL | BDF_ORIENTATION_FLIPPED, 2, HARDWARE_PREFIX_KONAMI,
