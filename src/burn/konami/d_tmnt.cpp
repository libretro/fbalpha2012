...
unsigned short __fastcall Blswhstl68KReadWord(unsigned int a)
{
	if (a >= 0x300000 && a <= 0x303fff) {
		...
		if (Offset & 0x31) {
			return swapWord(SpriteRam[Offset]);
		} else {
...
static void tmnt2_protection_write()
{
	...
	if ((swapWord(mcu[8]) & 0xff00) != 0x8200) return;

	src_addr = (swapWord(mcu[0]) | (swapWord(mcu[1])&0xff)<<16) >> 1;
	dst_addr = (swapWord(mcu[2]) | (swapWord(mcu[3])&0xff)<<16) >> 1;
	mod_addr = (swapWord(mcu[4]) | (swapWord(mcu[5])&0xff)<<16) >> 1;
	zlock    = (swapWord(mcu[8]) & 0xff) == 0x0001;
	...
	if (!xlock)
	{
		...
		{
			i >>= 8;
			xoffs += (int)(pow((float)i, (float)/*1.898461*/1.891292) * xoffs / 599.250121);
	}
		else if (i < 0)
		{
	...
	if (!ylock)
	{
		...
		{
			i >>= 8;
			yoffs += (int)(pow((float)i, (float)/*1.898461*/1.891292) * yoffs / 599.250121);
			}
		else if (i < 0)
		{
...
{
	*((unsigned short*)(Drv68KRam + 0x4800 + (a & 0x1e))) = swapWord(d);
}
...
void __fastcall Ssriders68KWriteWord(unsigned int a, unsigned short d)
{
	...
	if ((a & 0xffff80) == 0x1c0500) {
		*((unsigned short*)(Drv68KRam + 0x4000 + (a & 0x7e))) = swapWord(d);
		return;
	}
...
static void DrvCalcPalette()
{
	...
	for (int i = 0; i < 0x800; i += 2) {
		int Offset = i & ~1;
		UINT32 Data = (swapWord(PaletteRam[Offset]) << 8) | swapWord(PaletteRam[Offset + 1]);
...
static void BlswhstlCalcPalette()
{
	...
	for (int i = 0; i < 0x800; i++) {
		UINT32 Data = swapWord(PaletteRam[i]);
...
struct BurnDriver BurnDrvTmnt = {
	...
	NULL, NULL, NULL, NULL,
	BDF_GAME_WORKING, 4, HARDWARE_KONAMI_68K_Z80,
...
struct BurnDriver BurnDrvTmntu = {
	...
	NULL, NULL, NULL, NULL,
	BDF_GAME_WORKING | BDF_CLONE, 4, HARDWARE_KONAMI_68K_Z80,
...
struct BurnDriver BurnDrvTmht2p = {
	...
	NULL, NULL, NULL, NULL,
	BDF_GAME_WORKING | BDF_CLONE, 2, HARDWARE_KONAMI_68K_Z80,
...
struct BurnDriver BurnDrvMia = {
	...
	NULL, NULL, NULL, NULL,
	BDF_GAME_WORKING, 2, HARDWARE_KONAMI_68K_Z80,
...
struct BurnDriver BurnDrvBlswhstl = {
	...
	NULL, NULL, NULL, NULL,
	BDF_GAME_WORKING | BDF_ORIENTATION_VERTICAL | BDF_ORIENTATION_FLIPPED, 2, HARDWARE_KONAMI_68K_Z80,
...
struct BurnDriver BurnDrvDetatwin = {
	...
	NULL, NULL, NULL, NULL,
	BDF_GAME_WORKING | BDF_CLONE | BDF_ORIENTATION_VERTICAL | BDF_ORIENTATION_FLIPPED, 2, HARDWARE_KONAMI_68K_Z80,
...
struct BurnDriver BurnDrvSunsetbl = {
	...
	NULL, NULL, NULL, NULL,
	BDF_CLONE, 2, HARDWARE_KONAMI_68K_Z80,
