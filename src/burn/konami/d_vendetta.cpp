...
static void DrvRecalcPal()
{
	...
	for (int i = 0; i < 0x1000 / 2; i++) {
		unsigned short d = (swapWord(p[i]) << 8) | (swapWord(p[i]) >> 8);
...
struct BurnDriver BurnDrvVendetta = {
	...
	VendettaInit, DrvExit, DrvFrame, DrvDraw, DrvScan, &DrvRecalc,
	288, 224, 4, 3
};
...
struct BurnDriver BurnDrvEsckids = {
	...
	EsckidsInit, DrvExit, DrvFrame, DrvDraw, NULL, &DrvRecalc,
	272, 224, 4, 3
};
...
struct BurnDriver BurnDrvEsckidsj = {
	...
	EsckidsInit, DrvExit, DrvFrame, DrvDraw, DrvScan, &DrvRecalc,
	272, 224, 4, 3
};
