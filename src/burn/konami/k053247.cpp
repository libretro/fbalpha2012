...
void K053247Write(int offset, int data)
{
	if (data & 0x10000) { // use word
		*((unsigned short*)(K053247Ram + (offset & 0xffe))) = swapWord(data);
	} else {
...
void K053246Write(int offset, int data)
{
	if (data & 0x10000) { // handle it as a word
		*((unsigned short*)(K053246Regs + (offset & 6))) = swapWord(data);
	} else {
...
#define NUM_SPRITES 256
	...
	if (zcode == -1)
	{
		for (; offs<0x800; offs+=8)
			if (swapWord(SprRam[offs]) & 0x8000) sortedlist[count++] = offs;
	}
	else
	{
		for (; offs<0x800; offs+=8)
			if ((swapWord(SprRam[offs]) & 0x8000) && ((swapWord(SprRam[offs]) & 0xff) != zcode)) 
	...
	if (!(K053247Regs[0xc/2] & 0x10))
	{
		...
		{
			offs = sortedlist[y];
			zcode = swapWord(SprRam[offs]) & 0xff;
			for (x=y+1; x<w; x++)
			{
				temp = sortedlist[x];
				code = swapWord(SprRam[temp]) & 0xff;
	...
	for (int i = 0; i <= count; i++)
	{
		...
		code = swapWord(SprRam[offs+1]);
		shadow = color = swapWord(SprRam[offs+6]);
		primask = 0;
		...
		oy = (short)swapWord(SprRam[offs+2]);
		ox = (short)swapWord(SprRam[offs+3]);
		...
		y = zoomy = swapWord(SprRam[offs+4]) & 0x3ff;
		if (zoomy) zoomy = (0x400000+(zoomy>>1)) / zoomy; else zoomy = 0x800000;
		if (!(temp & 0x4000))
		{
			x = zoomx = swapWord(SprRam[offs+5]) & 0x3ff;
