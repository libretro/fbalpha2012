  ...
  //         (Pico.video.status&8)||!(Pico.video.reg[1]&0x40), Pico.m.scanline, SekCyclesDone());
	...
	case 3: // cram
		...
		for(a2 = a&0x7f; len; len--) {
			CalcCol( a2>>1, swapWord((unsigned short)(*pd)) );
			pd++;
			// AutoIncrement
			a2+=inc;
			// didn't src overlap?
...
unsigned short __fastcall MegadriveVideoReadWord(unsigned int sekAddress)
{
	...
	case 0x00:	// data
		switch (RamVReg->type) {
			case 0: res = swapWord(RamVid [(RamVReg->addr >> 1) & 0x7fff]); break;
			case 4: res = swapWord(RamSVid[(RamVReg->addr >> 1) & 0x003f]); break;
			case 8: res = swapWord(RamPal [(RamVReg->addr >> 1) & 0x003f]); break;
		}
		RamVReg->addr += RamVReg->reg[0xf];
		break;
	...
	case 0x00:	// data
		...
		if ((RamVReg->command & 0x80) && (RamVReg->reg[1]&0x10) && (RamVReg->reg[0x17]>>6)==2) {
			...
			case 3: 
				...
				break;
			case 5:
				RamSVid[(RamVReg->addr >> 1) & 0x003f] = swapWord(wordValue); 
				break;
			}
			//dprintf("w[%i] @ %04x, inc=%i [%i|%i]", Pico.video.type, a, Pico.video.reg[0xf], Pico.m.scanline, SekCyclesDone());
			//AutoIncrement();
...
static int TileNorm(int sx,int addr,int pal)
{
	...
	pack = swapLong(*(unsigned int *)(RamVid + addr)); // Get 8 pixels
	if (pack) {
...
static void DrawStrip(struct TileStrip *ts, int sh)
{
	...
	for (; cells; dx+=8,tilex++,cells--) {
		...
		code=swapWord(RamVid[ts->nametab + (tilex&ts->xmask)]);
...
static void DrawStripVSRam(struct TileStrip *ts, int plane)
{
	...
	if(dx != 8) {
		...
		vscroll = swapWord(RamSVid[plane]);
	...
	for (; cell < ts->cells; dx+=8,tilex++,cell++) {
		...
		if((cell&1)==0) {
			int line,vscroll;
			vscroll = swapWord(RamSVid[plane+(cell&~1)]);
		...
		code = swapWord(RamVid[ts->nametab + nametabadd + (tilex&ts->xmask)]);
		if (code==blank) continue;
		if (code>>15) { // high priority tile
			int cval = code | (dx<<16) | (ty<<25);
			if(code&0x1000) cval^=7<<26;
...
static void DrawLayer(int plane, int *hcache, int maxcells, int sh)
{
	...
	ts.hscroll = swapWord(RamVid[htab & 0x7fff]);

	if((RamVReg->reg[12]&6) == 6) {
		// interlace mode 2
		vscroll = swapWord(RamSVid[plane]); // Get vertical scroll value
...
static void DrawWindow(int tstart, int tend, int prio, int sh)
{
	...
	if(!(rendstatus & 2)) {
		...
		code = swapWord(RamVid[nametab + tilex]);
...
static void DrawAllSpritesInterlace(int pri, int maxwidth)
{
	...
	for (i=u=0; u < 80 && i < 21; u++) {
		...
		// get sprite info
		code = swapLong(sprite[0]);
		sx = swapLong(sprite[1]);
...
#if defined (_XBOX) || defined (SN_TARGET_PS3)
#define TOTAL_68K_CYCLES_PAL	TOTAL_68K_CYCLES
#define TOTAL_Z80_CYCLES_PAL	TOTAL_Z80_CYCLES
#else
