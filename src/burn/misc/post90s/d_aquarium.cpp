...
static void draw_bg_layer(unsigned char *src, unsigned char *gfx_base, int scrollx, int scrolly, int transp, int priority)
{
	...
	for (int offs = 0; offs < 32 * 32; offs++)
	{
		...
		int code  = swapWord(vram[offs * 2]) & 0x0fff;
...
static void draw_tx_layer(int scrollx, int scrolly)
{
	...
	for (int offs = 0; offs < 64 * 64; offs++)
	{
		...
		int code = swapWord(vram[offs]) & 0xfff;
		int color =swapWord(vram[offs]) >> 12;
...
static void draw_sprites()
{
	...
	{
		int code = (swapWord(sram[offs+5]) & 0xff) | ((swapWord(sram[offs+6]) & 0x3f) << 8);

		if (~swapWord(sram[offs+4]) & 0x80)
		{
			int x = (((swapWord(sram[offs+0]) & 0xff) | ((swapWord(sram[offs+1]) & 0x7f) << 8)) - ((swapWord(sram[offs+1]) & 0x80) << 9)) - 16;
			int y = (((swapWord(sram[offs+2]) & 0xff) | ((swapWord(sram[offs+3]) & 0x7f) << 8)) - ((swapWord(sram[offs+3]) & 0x80) << 9)) - 16;

			int col  =  swapWord(sram[offs+7]) & 0x0f;
			int chain = swapWord(sram[offs+4]) & 0x07;
			int flipy = swapWord(sram[offs+4]) & 0x10;
			int flipx = swapWord(sram[offs+4]) & 0x20;

			if (~swapWord(sram[offs+4]) & 0x08) {
				if (flipy) y += chain << 4;
				if (flipx) x += chain << 4;
			}
