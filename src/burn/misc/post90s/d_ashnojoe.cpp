...
static void draw_16x16_layer(unsigned char *ram, unsigned char *gfx, int color_off, int scroll_off, int mask, int transparent)
{
	...
	for (int offs = 0; offs < 32 * 32; offs++)
	{
		...
		int code  =   swapWord(vram[offs * 2 + 0]) & mask;
		int color = ((swapWord(vram[offs * 2 + 1]) >> 8) & 0x1f) + color_off; 
...
static void draw_8x8_layer(unsigned char *ram, unsigned char *gfx, int color_off, int scroll_off, int high)
{
	...
	for (int offs = 0; offs < 64 * (32 << high); offs++)
	{
		...
		int color = (swapWord(vram[offs]) >> 12) + color_off; 
...
struct BurnDriver BurnDrvAshnojoe = {
	...
	NULL, ashnojoeRomInfo, ashnojoeRomName, AshnojoeInputInfo, AshnojoeDIPInfo,
	DrvInit, DrvExit, DrvFrame, DrvDraw, DrvScan, NULL,
	288, 208, 4, 3
};
