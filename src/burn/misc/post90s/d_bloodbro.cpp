...
static void draw_layer(unsigned char *src, int palette_offset, int transp, int scrollx, int scrolly)
{
	...
	for (int offs = 0; offs < 32 * 16; offs++)
	{
		...
		int code = (swapWord(vram[offs]) & 0xfff) | (transp << 12);
...
static void draw_sprites(int priority)
{
	...
	{
		int attr = swapWord(ram[offs]);
		int prio = (attr & 0x0800) >> 11;
		if (attr & 0x8000 || prio != priority) continue;

		int width   = (attr >> 7) & 7;
		int height  = (attr >> 4) & 7;
		int code    = swapWord(ram[offs+1]) & 0x1fff;
		int sx      = swapWord(ram[offs+2]) & 0x01ff;
		int sy      = swapWord(ram[offs+3]) & 0x01ff;
		if (sx > 255) sx -= 512;
		if (sy > 255) sy -= 512;
		sy -= 16;
...
static int DrvDraw()
{
	...
	draw_layer(DrvBgRAM, 0x400, 0, swapWord(scroll[0]) & 0x1ff, swapWord(scroll[1]) & 0x0ff);

	draw_sprites(1);

	draw_layer(DrvFgRAM, 0x500, 1, swapWord(scroll[2]) & 0x1ff, swapWord(scroll[3]) & 0x0ff);
...
struct BurnDriver BurnDrvBloodbrob = {
	"bloodbrob", "bloodbro", NULL, "1990",
	"Blood Bros. (set 3)\0", NULL, "Tad", "Miscellaneous",
	NULL, NULL, NULL, NULL,
	BDF_GAME_WORKING | BDF_CLONE, 2, HARDWARE_MISC_MISC,
	NULL, bloodbrobRomInfo, bloodbrobRomName, BloodbroInputInfo, BloodbroDIPInfo,
	DrvInit, DrvExit, DrvFrame, DrvDraw, DrvScan, &DrvRecalc,
	256, 224, 4, 3
};
...
struct BurnDriver BurnDrvWeststry = {
