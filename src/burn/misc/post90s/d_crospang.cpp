...
static inline int crospangSynchroniseStream(int nSoundRate)
{
...
static void draw_sprites()
{
	...
	{
		int sprite = swapWord(ram[offs + 1]) & 0x7fff;
		if (!sprite) continue;

		int y = swapWord(ram[offs]);
		if ((y & 0x1000) && (GetCurrentFrame() & 1)) continue; // flash

		int x = swapWord(ram[offs + 2]);
		int color = (x >>9) & 0xf;
...
static int DrvDraw()
{
	if (DrvRecalc) {
		...
		for (int i = 0; i < 0x300; i++) {
			...
			r = (swapWord(pal[i]) >> 10) & 0x1f;
			g = (swapWord(pal[i]) >>  5) & 0x1f;
			b = (swapWord(pal[i]) >>  0) & 0x1f;
...
struct BurnDriver BurnDrvCrospang = {
	...
	crospangInit, DrvExit, DrvFrame, DrvDraw, DrvScan, &DrvRecalc,
	320, 240, 4, 3
};
...
struct BurnDriver BurnDrvHeuksun = {
	...
	heuksunInit, DrvExit, DrvFrame, DrvDraw, DrvScan, &DrvRecalc,
	320, 240, 4, 3
};
...
struct BurnDriver BurnDrvBestri = {
	...
	bestriInit, DrvExit, DrvFrame, DrvDraw, DrvScan, &DrvRecalc,
	320, 240, 4, 3
};
