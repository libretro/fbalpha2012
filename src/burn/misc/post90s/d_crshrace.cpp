...
static void draw_sprites()
{
	...
	while (offs < 0x0400 && (swapWord(sprbuf1[offs]) & 0x4000) == 0)
	{
		int attr_start = 4 * (swapWord(sprbuf1[offs++]) & 0x03ff);

		int ox        =  swapWord(sprbuf1[attr_start + 1]) & 0x01ff;
		int xsize     = (swapWord(sprbuf1[attr_start + 1]) & 0x0e00) >> 9;
		int zoomx     = (swapWord(sprbuf1[attr_start + 1]) & 0xf000) >> 12;
		int oy        =  swapWord(sprbuf1[attr_start + 0]) & 0x01ff;
		int ysize     = (swapWord(sprbuf1[attr_start + 0]) & 0x0e00) >> 9;
		int zoomy     = (swapWord(sprbuf1[attr_start + 0]) & 0xf000) >> 12;
		int flipx     =  swapWord(sprbuf1[attr_start + 2]) & 0x4000;
		int flipy     =  swapWord(sprbuf1[attr_start + 2]) & 0x8000;
		int color     = (swapWord(sprbuf1[attr_start + 2]) & 0x1f00) >> 8;
		int map_start =  swapWord(sprbuf1[attr_start + 3]) & 0x7fff;

		zoomx = 16 - zoomtable[zoomx]/8;
		zoomy = 16 - zoomtable[zoomy]/8;

		if (swapWord(sprbuf1[attr_start + 2]) & 0x20ff) color = 1; // what?? mame_rand? why?
		...
		for (int y = 0;y <= ysize;y++)
		{
			...
			for (int x = 0;x <= xsize;x++)
			{
				...
				int code = swapWord(sprbuf2[map_start & 0x7fff]) & 0x7fff;
				map_start++;
...
static int DrvDraw()
{
	if (DrvRecalc) {
		...
		for (int i = 0; i < 0x400; i++) {
			unsigned short p_be = swapWord(p[i]);
			r = (p_be >>  0) & 0x1f;
			g = (p_be >> 10) & 0x1f;
			b = (p_be >>  5) & 0x1f;
...
struct BurnDriver BurnDrvCrshrace2 = {
	...
	NULL, crshrace2RomInfo, crshrace2RomName,  CrshraceInputInfo,  CrshraceDIPInfo,
	DrvInit, DrvExit, DrvFrame, DrvDraw, DrvScan, &DrvRecalc,
	224, 320, 3, 4
};
