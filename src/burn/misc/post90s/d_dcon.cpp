...
static void draw_sprites(int priority, int yoffset)
{
	...
	for (int offs = 0x400; offs >= 0; offs -= 4)
	{
		...
		int sprite = swapWord(ram[offs+1]);
...
struct BurnDriver BurnDrvDcon = {
	...
	DrvInit, DrvExit, DrvFrame, DconDraw, DrvScan, &DrvRecalc,
	320, 224, 4, 3
};
...
struct BurnDriver BurnDrvSdgndmps = {
	...
	SdgndmpsInit, DrvExit, DrvFrame, SdgndmpsDraw, DrvScan, &DrvRecalc,
	320, 224, 4, 3
};
