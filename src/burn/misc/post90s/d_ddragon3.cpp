...
static struct BurnRomInfo DrvpRomDesc[] = {
	...
	{ "15.ic46",       0x20000, 0x24d0bf41, BRF_GRA },	     //  4	Tiles
	{ "30.ic13",       0x20000, 0x72fe2b16, BRF_GRA },	     //  5	Tiles
	{ "31.ic14",       0x20000, 0xab48a0c8, BRF_GRA },	     //  6	Tiles
	{ "23.ic29",       0x20000, 0x0768fedd, BRF_GRA },	     //  7	Tiles
	{ "24.ic30",       0x20000, 0xec9db18a, BRF_GRA },	     //  8	Tiles
	{ "21.ic25",       0x20000, 0x902744b9, BRF_GRA },	     //  9	Tiles
	{ "22.ic26",       0x20000, 0x5b142d4d, BRF_GRA },	     //  10	Tiles
	...
	{ "30a12-0.ic43",  0x20000, 0x91da004c, BRF_GRA },	     //  15
	{ "4.ic33",        0x20000, 0x8c71eb06, BRF_GRA },	     //  16
	{ "5.ic34",        0x20000, 0x3e134be9, BRF_GRA },	     //  17
	{ "6.ic35",        0x20000, 0xb4115ef0, BRF_GRA },	     //  18
	{ "7.ic36",        0x20000, 0x4639333d, BRF_GRA },	     //  19
	{ "30a11-0.ic37",  0x20000, 0x5f419232, BRF_GRA },	     //  20
	{ "16.ic19",       0x20000, 0x04420cc8, BRF_GRA },	     //  21
	{ "17.ic20",       0x20000, 0x33f97b2f, BRF_GRA },	     //  22
	{ "18.ic21",       0x20000, 0x0f9a8f2a, BRF_GRA },	     //  23
	{ "19.ic22",       0x20000, 0x15c91772, BRF_GRA },	     //  24
	{ "30a10-0.ic23",  0x20000, 0x12f641ba, BRF_GRA },	     //  25
	{ "25.ic3",        0x20000, 0x894734b3, BRF_GRA },	     //  26
	{ "26.ic4",        0x20000, 0xcd504584, BRF_GRA },	     //  27
	{ "27.ic5",        0x20000, 0x38e8a9ad, BRF_GRA },	     //  28
	{ "28.ic6",        0x20000, 0x80c1cb74, BRF_GRA },	     //  29
...
static int DrvInit()
{
	...
	BurnSetRefreshRate(280000/4/44.8/27.2);
...
static int DrvpInit()
{
	...
	nRet = BurnLoadRom(DrvTempRom + 0x080000, 15, 1); if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x100000, 16, 1); if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x120000, 17, 1); if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x140000, 18, 1); if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x160000, 19, 1); if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x180000, 20, 1); if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x200000, 21, 1); if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x220000, 22, 1); if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x240000, 23, 1); if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x260000, 24, 1); if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x280000, 25, 1); if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x300000, 26, 1); if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x320000, 27, 1); if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x340000, 28, 1); if (nRet != 0) return 1;
	nRet = BurnLoadRom(DrvTempRom + 0x360000, 29, 1); if (nRet != 0) return 1;
...
static inline unsigned int CtribeCalcCol(unsigned short nColour)
{
	int r, g, b;
...
static void DrvRenderFgLayer(int Opaque)
{
	...
	for (my = 0; my < 32; my++) {
		for (mx = 0; mx < 32; mx++) {
			...
			Code = swapWord(VideoRam[Offset + 1]) & 0x1fff;
			Colour = Attr & 0x0f;
			xFlip = Attr & 0x40;
...
static void DrvRenderSprites()
{
	...
	while (Source < Finish) {
		UINT16 Attr = swapWord(Source[1]);

		if (Attr & 0x01) {
			int i;
			int Bank = swapWord(Source[3]) & 0xff;
			int Code = (swapWord(Source[2]) & 0xff) + (Bank * 256);
			int Colour = swapWord(Source[4]) & 0xf;
			int xFlip = Attr & 0x10;
			int yFlip = Attr & 0x08;
			int sx = swapWord(Source[5]) & 0xff;
			int sy = swapWord(Source[0]) & 0xff;
