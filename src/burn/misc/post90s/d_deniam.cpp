...
static inline int deniam16ZetSynchroniseStream(int nSoundRate)
{
	return (long long)(ZetTotalCycles() * nSoundRate / 6250000);
}

static inline int deniam16SekSynchroniseStream(int nSoundRate)
{
...
static void draw_sprites(int priority)
{
	...
	{
		if (priority != ((swapWord(source[offs+4])&0xc0) >> 6)) continue;

		int sx = (swapWord(source[offs+1]) & 0x01ff) + 16*8 - 1;
		if (sx >= 512) sx -= 512;
		sx -= 192;
		int starty = swapWord(source[offs+0]) & 0xff;
		if (starty >= nScreenHeight) continue;
		int endy = swapWord(source[offs+0]) >> 8;
		if (endy >= nScreenHeight) endy = nScreenHeight-1;

		int width = swapWord(source[offs+2]) & 0x007f;
		int flipx = swapWord(source[offs+2]) & 0x0100;
		if (flipx) sx++;

		int color = (0x40 + (swapWord(source[offs+4]) & 0x3f)) << 4;

		int start = swapWord(source[offs+3]) + ((swapWord(source[offs+4]) & 0x1f00) << 8);
...
static void draw_layer(int *in_page, int scrollx, int scrolly, int transp)
{
	...
	for (int offs = 0; offs < 128 * 64; offs++)
	{
		...
		int attr = swapWord(vram[in_page[ofst>>11] * 0x0800 + (ofst & 0x7ff)]);
		int code = attr;
...
static void draw_text()
{
	...
	for (int offs = 0; offs < 64 * 32; offs++)
	{
		...
		int attr = swapWord(vram[DrvTxtTable[offs]]);
...
static int DrvDraw()
{
	...
	//	draw_sprites(0);
		...
		int bg_scrollx = (swapWord(txtram[bg_scrollx_reg]) - bg_scrollx_offs) & 0x1ff;
		int bg_scrolly = ((swapWord(txtram[bg_scrolly_reg]) & 0xff) - bg_scrolly_offs) & 0xff;
		int page = swapWord(txtram[bg_page_reg]);
		...
		int fg_scrollx = (swapWord(txtram[fg_scrollx_reg]) - fg_scrollx_offs) & 0x1ff;
		int fg_scrolly = ((swapWord(txtram[fg_scrolly_reg]) & 0xff) - fg_scrolly_offs) & 0xff;
...
struct BurnDriver BurnDrvLogicpro = {
	...
	logicproInit, DrvExit, DrvFrame, DrvDraw, DrvScan, &DrvRecalc,
	320, 224, 4, 3
};
...
struct BurnDriver BurnDrvKarianx = {
	...
	karianxInit, DrvExit, DrvFrame, DrvDraw, DrvScan, &DrvRecalc,
	320, 224, 4, 3
};
...
struct BurnDriver BurnDrvLogicpr2 = {
	...
	logicpr2Init, DrvExit, DrvFrame, DrvDraw, DrvScan, &DrvRecalc,
	320, 224, 4, 3
};
