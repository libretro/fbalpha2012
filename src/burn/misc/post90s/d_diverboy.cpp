...
static void draw_sprites()
{
	...
	{
		int sy   = ( 256 - swapWord(vram[offs + 4])) - 16;
		int sx   = ((480 - swapWord(vram[offs + 0])) & 0x1ff) - 173;
		int attr = swapWord(vram[offs + 1]);
		int code = swapWord(vram[offs + 3]);
...
static int DrvDraw()
{
	if (DrvRecalc) {
		...
		for (int i = 0; i < 0x800 / 2; i++, pal++) {
			r = ((swapWord(*pal) << 4) & 0xf0) | ((swapWord(*pal) << 0) & 0x0f);
			g = ((swapWord(*pal) >> 0) & 0xf0) | ((swapWord(*pal) >> 4) & 0x0f);
			b = ((swapWord(*pal) >> 4) & 0xf0) | ((swapWord(*pal) >> 8) & 0x0f);
