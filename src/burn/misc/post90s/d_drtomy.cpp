...
static void draw_sprites()
{
	...
	for (i = 3; i < 0x1000/2; i+=4){
		int sx = swapWord(spriteram[i+2]) & 0x01ff;
		int sy = (240 - (swapWord(spriteram[i]) & 0x00ff)) & 0x00ff;
		int number = swapWord(spriteram[i+3]);
		int color = (swapWord(spriteram[i+2]) & 0x1e00) >> 9;
		int attr = (swapWord(spriteram[i]) & 0xfe00) >> 9;
...
static int DrvDraw()
{
...

	{
		...
		for (int offs = 0; offs < 0x400 >> 1; offs++) {
			...
			// background
			code  = swapWord(bg_ram[offs]) & 0xfff;
			color = swapWord(bg_ram[offs]) >> 12;

			Render16x16Tile_Clip(pTransDraw, code, sx, sy, color, 4, 0, Gfx1);

			// foreground
			code  = swapWord(fg_ram[offs]) & 0xfff;
			color = swapWord(fg_ram[offs]) >> 12;
