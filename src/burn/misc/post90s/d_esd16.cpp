...
void __fastcall hedpanic_write_word(unsigned int address, unsigned short data)
{
	...
	switch (address)
	{
		...
		case 0xd00008:
			...
			*((unsigned short*)(DrvVidRAM1 + ofst)) = swapWord(data);
		return;
	}
	return;
}
...
static void esd16_draw_sprites(int priority)
{
	...
	for (int offs = 0;  offs < 0x800/2 - 8/2 ; offs += 8/2 )
	{
		...
		int sx = swapWord(spriteram16[ offs + 2 ]);

		if ((sx >> 15) != priority) continue;

		int	sy	=	swapWord(spriteram16[ offs ]);
		int	code	=	swapWord(spriteram16[ offs + 1 ]);
...
static void draw_background_16x16(unsigned char *vidram, int color, int transp, int scrollx, int scrolly)
{
	...
	for (offs = 0; offs < 0x1000 / 2; offs++) {
		int code = swapWord(vram[offs]) & 0x3fff;
...
static struct BurnRomInfo multchmpkRomDesc[] = {
...
STD_ROM_PICK(multchmpk)
STD_ROM_FN(multchmpk)

struct BurnDriver BurnDrvMultchmpk = {
	"multchmpk", "multchmp", NULL, "1999",
	"Multi Champ (Korea)\0", NULL, "ESD", "Miscellaneous",
	NULL, NULL, NULL, NULL,
	BDF_GAME_WORKING | BDF_CLONE, 2, HARDWARE_MISC_MISC,
	NULL, multchmpkRomInfo, multchmpkRomName, MultchmpInputInfo, MultchmpDIPInfo,
	MultchmpInit, DrvExit, DrvFrame, DrvDraw, DrvScan,
	&DrvRecalc, 320, 240, 4, 3
};
...
static struct BurnRomInfo hedpanicfRomDesc[] = {
...
STD_ROM_PICK(hedpanicf)
STD_ROM_FN(hedpanicf)

struct BurnDriver BurnDrvHedpanicf = {
	...
	NULL, NULL, NULL, NULL,
	BDF_GAME_WORKING | BDF_CLONE, 2, HARDWARE_MISC_MISC,
	NULL, hedpanicfRomInfo, hedpanicfRomName, HedpanicInputInfo, NULL,
	HedpanicInit, DrvExit, DrvFrame, DrvDraw, DrvScan,
	&DrvRecalc, 320, 240, 4, 3
};
...
static struct BurnRomInfo hedpanicoRomDesc[] = {
	{ "esd2.rom",	0x040000, 0x70b08424, 1 | BRF_PRG | BRF_ESS },		//  0 - 68k Code
	{ "esd1.rom", 	0x040000, 0x4e0682c5, 1 | BRF_PRG | BRF_ESS },		//  1

	{ "esd3.su06",	0x040000, 0xa88d4424, 2 | BRF_PRG | BRF_ESS },		//  2 - Z80 Code

	{ "sm2.ju02",	0x200000, 0x7a9610e4, 3 | BRF_GRA },			//  3
	{ "sm1.ju01",	0x200000, 0x8083813f, 3 | BRF_GRA },			//  4 - Sprites
	{ "esd5.rom",	0x080000, 0x82c5727f, 3 | BRF_GRA },			//  5

	{ "sm3.fu35",	0x200000, 0x94dd4cfc, 4 | BRF_GRA },			//  6 - Tiles
	{ "sm4.fu34",	0x200000, 0x6da0fb9e, 4 | BRF_GRA },			//  7

	{ "esd4.rom",	0x020000, 0xd7ca6806, 5 | BRF_SND },			//  8 - OKI Samples
};

STD_ROM_PICK(hedpanico)
STD_ROM_FN(hedpanico)

struct BurnDriver BurnDrvHedpanico = {
	"hedpanico", "hedpanic", NULL, "2000",
	"Head Panic (ver. 0615, 15/06/1999)\0", "Story line in Japanese, game instructions in English", "ESD", "Miscellaneous",
	NULL, NULL, NULL, NULL,
	BDF_GAME_WORKING | BDF_CLONE, 2, HARDWARE_MISC_MISC,
	NULL, hedpanicoRomInfo, hedpanicoRomName, HedpanicInputInfo, NULL,
	HedpanicInit, DrvExit, DrvFrame, DrvDraw, DrvScan,
	&DrvRecalc, 320, 240, 4, 3
};
...
static struct BurnRomInfo mchampdxaRomDesc[] = {
...
STD_ROM_PICK(mchampdxa)
STD_ROM_FN(mchampdxa)

struct BurnDriver BurnDrvMchampdxa = {
	"mchampdxa", "mchampdx", NULL, "1999",
	"Multi Champ Deluxe (ver. 1126, 26/11/1999)\0", NULL, "ESD", "Miscellaneous",
	NULL, NULL, NULL, NULL,
	BDF_GAME_WORKING | BDF_CLONE, 2, HARDWARE_MISC_MISC,
	NULL, mchampdxaRomInfo, mchampdxaRomName, HedpanicInputInfo, NULL,
	MchampdxInit, DrvExit, DrvFrame, DrvDraw, DrvScan,
	&DrvRecalc, 320, 240, 4, 3
};
