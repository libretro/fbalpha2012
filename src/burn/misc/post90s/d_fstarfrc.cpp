...
int FstarfrcInit()
{
	...
	if (!strcmp(BurnDrvGetTextA(DRV_NAME), "fstarfrc")
		|| !strcmp(BurnDrvGetTextA(DRV_NAME), "fstarfrcj")) {
...
static void draw_sprites(int layerpriority)
{
	...
	{
		if (swapWord(pFstarfrcSpriteRam[offs]) & 0x04)	/* enable */
		{
			int code,color,sizex,sizey,flipx,flipy,xpos,ypos;
			int x,y,priority;

			code = swapWord(pFstarfrcSpriteRam[offs+1]);
			color = (swapWord(pFstarfrcSpriteRam[offs+2]) & 0xf0) >> 4;
			sizex = 1 << ((swapWord(pFstarfrcSpriteRam[offs+2]) & 0x03) >> 0);
			if (Riot) {
				sizey = sizex;
			}
			else {
				sizey = 1 << ((swapWord(pFstarfrcSpriteRam[offs+2]) & 0x0c) >> 2);
			}
			...
			flipx = swapWord(pFstarfrcSpriteRam[offs]) & 0x01;
			flipy = swapWord(pFstarfrcSpriteRam[offs]) & 0x02;
			xpos = swapWord(pFstarfrcSpriteRam[offs+4]);
			if (xpos >= 0x8000) xpos -= 0x10000;
			ypos = swapWord(pFstarfrcSpriteRam[offs+3]);
			if (ypos >= 0x8000) ypos -= 0x10000;
			priority = (swapWord(pFstarfrcSpriteRam[offs]) & 0xc0) >> 6;
