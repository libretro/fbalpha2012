...
static int GalpanicDraw()
{
 	...
 	for (int i = 0; i < 1024; i++) {
 		unsigned short nColour = swapWord(RamPal[i]);
	...
	for (int j=0;j<224;j++) {
		for (int i=0;i<256;i++) {
			if (swapWord(*f))	*d = RamCurPal[swapWord(*f)];
			else 	*d = RamCTB64k[swapWord(*s) >> 1];
			d--;
			s++;
			f++;
		}
	...
	for (int offs=0; offs<0x002400; offs+=0x08) {
		...
		attr1 = swapWord(RamSpr[offs + 3]);
		x = swapWord(RamSpr[offs + 4]) - ((attr1 & 0x01) << 8);
		y = swapWord(RamSpr[offs + 5]) + ((attr1 & 0x02) << 7);
		...
		attr2 = swapWord(RamSpr[offs + 7]);
		code = swapWord(RamSpr[offs + 6]) + ((attr2 & 0x1f) << 8);
		flipx = attr2 & 0x80;
		flipy = attr2 & 0x40;
...
static int ComadDraw()
{
	...
	for (int offs=0; offs<0x000800; offs+=4) {
		...
		code = swapWord(RamSpr[offs + 1]) & 0x1fff;
		color = (swapWord(RamSpr[offs]) & 0x003c) << 2;
		flipx = swapWord(RamSpr[offs]) & 0x0002;
		flipy = swapWord(RamSpr[offs]) & 0x0001;

		if((RamSpr[offs] & 0x6000) == 0x6000) /* Link bits */
		{
			sx += swapWord(RamSpr[offs + 2]) >> 6;
			sy += swapWord(RamSpr[offs + 3]) >> 6;
		}
		else
		{
			sx = swapWord(RamSpr[offs + 2]) >> 6;
...
struct BurnDriver BurnDrvWownfant = {
	...
	WownfantInit, GalpanicExit, ComadFrame, ComadDraw, GalpanicScan, &RecalcBgPalette,
	256, 224, 4, 3
};
...
struct BurnDriver BurnDrvZipzap = {
	...
	NULL, NULL, NULL, NULL,
	BDF_ORIENTATION_VERTICAL | BDF_16BIT_ONLY, 2, HARDWARE_MISC_MISC,
