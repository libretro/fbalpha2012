...
inline void Kaneko16MakeInputs()
{
	...
	// Clear Opposites
	DrvClearOpposites(&Kaneko16Input[0]);
	DrvClearOpposites(&Kaneko16Input[1]);
}
...
static void BloodwarMCURun()
{
	...
	UINT16 MCUCommand = swapWord(MCURam[0x10/2]);
	UINT16 MCUOffset = swapWord(MCURam[0x12/2]) >> 1;
	UINT16 MCUData = swapWord(MCURam[0x14/2]);
	...
	switch (MCUCommand >> 8) {
		...
		case 0x03: {
			MCURam[MCUOffset + 0] = swapWord(0xff00 - (Kaneko16Dip[0] << 8));
			return;
		}
...
#else
			MCURam[MCUOffset + 0] = 0x4d4d;
			MCURam[MCUOffset + 1] = 0x3530;
			MCURam[MCUOffset + 2] = 0x3532;
			MCURam[MCUOffset + 3] = 0x542d;
			MCURam[MCUOffset + 4] = 0x594f;
			MCURam[MCUOffset + 5] = 0x4f42;
			MCURam[MCUOffset + 6] = 0x3158;
			MCURam[MCUOffset + 7] = 0x3939;
...
#else
			MCURam[MCUOffset + 0] = 0x5355;
			MCURam[MCUOffset + 1] = 0x4d4d;
			MCURam[MCUOffset + 2] = 0x3730;
			MCURam[MCUOffset + 3] = 0x3331;
			MCURam[MCUOffset + 4] = 0x542d;
			MCURam[MCUOffset + 5] = 0x3142;
			MCURam[MCUOffset + 6] = 0x3939;
			MCURam[MCUOffset + 7] = 0x2034;
#endif
			return;
		}
...
static int Kaneko16ParseSpriteType0(int i, struct tempsprite *s)
{
	...
	Attr = swapWord(SpriteRam[Offset + 0]);
	s->code = swapWord(SpriteRam[Offset + 1]);
	s->x = swapWord(SpriteRam[Offset + 2]);
	s->y = swapWord(SpriteRam[Offset + 3]);
	...
	s->yoffs = swapWord(Kaneko16SpriteRegs[0x08 + (xOffs * 2) + 1]);
	s->xoffs = swapWord(Kaneko16SpriteRegs[0x08 + (xOffs * 2) + 0]);

	s->yoffs -= swapWord(Kaneko16SpriteRegs[0x01]);
...
static void Kaneko16QueueTilesLayer(int Layer)
{
	...
	xScroll =  swapWord(LAYERREGS[xScrollReg]);
	xOffs = Kaneko16TilesXOffset + xOffs;
	yScroll = (( swapWord(LAYERREGS[yScrollReg]) >> 6) & 0x1ff) - Kaneko16TilesYOffset;

	mx = my = -1;
	for (y = 0; y < 512; y++) {
		if (!(y % 16)) my++;

		LineScroll = swapWord(VSCROLLRAM[y]);
		py = y - yScroll;
		...
		for (x = 0; x < 512; x++) {
			...
			Code = swapWord(VRAM[TileIndex + 1]);
			if (Code >= numTiles) continue;
			Attr = swapWord(VRAM[TileIndex + 0]);
			Priority = (Attr >> 8) & 7;
			Colour = (Attr >> 2) & 0x3f;
			Flip = Attr & 3;
	...
	for (my = 0; my < 32; my++) {
		for (mx = 0; mx < 32; mx++) {
			...
			if (Priority == PriorityDraw) {
				...
				x -= (xScroll >> 6) & 0x1ff;
				y -= ( swapWord(LAYERREGS[yScrollReg]) >> 6) & 0x1ff;
...
#define HANDLE_VSCROLL(_N_) \
	if (Layer##_N_##Enabled) { \
		...
		int LineScroll = swapWord((VSCROLLRAM[0])); \
		for (i = 0; i < 0x200; i++) { \
			if (swapWord((VSCROLLRAM[i])) != LineScroll) { \
				vScrollFast = 0; \
				break; \
			} \
		} \
...
void BerlwallFrameRender()
{
	...
	int xScroll0 = swapWord(Kaneko16Layer0Regs[2]);
	int xScroll1 = swapWord(Kaneko16Layer0Regs[0]);

	if (~swapWord(Kaneko16Layer0Regs[4]) & 0x1000) Layer0Enabled = 1;
	if (~swapWord(Kaneko16Layer0Regs[4]) & 0x0010) Layer1Enabled = 1;
	...
	if (swapWord(Kaneko16Layer0Regs[4]) & 0x800) {
		HANDLE_VSCROLL(0)
	}

	if (swapWord(Kaneko16Layer0Regs[4]) & 0x008) {
		HANDLE_VSCROLL(1)
	}
...
void BloodwarFrameRender()
{
	...
	int xScroll0 =  swapWord(Kaneko16Layer0Regs[2]);
	int xScroll1 =  swapWord(Kaneko16Layer0Regs[0]);
	int xScroll2 =  swapWord(Kaneko16Layer1Regs[2]);
	int xScroll3 =  swapWord(Kaneko16Layer1Regs[0]);

	if (~ swapWord(Kaneko16Layer0Regs[4]) & 0x1000) Layer0Enabled = 1;
	if (~ swapWord(Kaneko16Layer0Regs[4]) & 0x0010) Layer1Enabled = 1;
	if (~ swapWord(Kaneko16Layer1Regs[4]) & 0x1000) Layer2Enabled = 1;
	if (~ swapWord(Kaneko16Layer1Regs[4]) & 0x0010) Layer3Enabled = 1;
	...
	if (swapWord(Kaneko16Layer1Regs[4]) & 0x800) {
		HANDLE_VSCROLL(2)
	}

	if (swapWord(Kaneko16Layer1Regs[4]) & 0x008) {
		HANDLE_VSCROLL(3)
	}
...
struct BurnDriver BurnDrvMgcrystlo = {
...
struct BurnDriver BurnDrvMgcrystlj = {
