...
#else
	rom[0x14832/2]	=	0x0060;		// 014832: 6700 0006  beq     $1483a (protection)
	rom[0x1587e/2]	=	0x1060;		// 01587E: 6710       beq     $15890 (rom check)
#endif
...
#else
	rom[0x0A558/2]	=	0x1260;		// 0A558  beq.s   $A56C
	rom[0x0A8DC/2]	=	0x1260;		// 0A8DC  beq.s   $A8F0
	rom[0x0AC92/2]	=	0x1260;		// 0AC92  beq.s   $ACA6
	rom[0x124CC/2]	=	0x1260;		// 124CC  beq.s   $124E0
	rom[0x12850/2]	=	0x1260;		// 12850  beq.s   $12864
	rom[0x12C06/2]	=	0x1260;		// 12C06  beq.s   $12C1A
	rom[0x1862A/2]	=	0x1260;		// 1862A  beq.s   $1863E
	rom[0x189AE/2]	=	0x1260;		// 189AE  beq.s   $189C2
	rom[0x18D64/2]	=	0x1260;		// 18D64  beq.s   $18D78
	rom[0x230FC/2]	=	0x1260;		// 230FC  beq.s   $23110
	rom[0x23480/2]	=	0x1260;		// 23480  beq.s   $23494
	rom[0x23836/2]	=	0x1260;		// 23836  beq.s   $2384A
	rom[0x2BD0E/2]	=	0x1260;		// 2BD0E  beq.s   $2BD22
	rom[0x2C092/2]	=	0x1260;		// 2C092  beq.s   $2C0A6
	rom[0x2C448/2]	=	0x1260;		// 2C448  beq.s   $2C45C
#endif
...
#else
	rom[0x0A550/2]	=	0x1260;		// 0A558  beq.s   $A564
	rom[0x0A8D4/2]	=	0x1260;		// 0A8D4  beq.s   $A8E8
	rom[0x0AC8A/2]	=	0x1260;		// 0AC8A  beq.s   $AC9E
	rom[0x124B8/2]	=	0x1260;		// 124B8  beq.s   $124CC
	rom[0x1283C/2]	=	0x1260;		// 1283C  beq.s   $12850
	rom[0x12BF2/2]	=	0x1260;		// 12BF2  beq.s   $12C06
	rom[0x18616/2]	=	0x1260;		// 18616  beq.s   $1862A
	rom[0x1899A/2]	=	0x1260;		// 1899A  beq.s   $189AE
	rom[0x18D50/2]	=	0x1260;		// 18D50  beq.s   $18D64
	rom[0x230E8/2]	=	0x1260;		// 230E8  beq.s   $230FC
	rom[0x2346C/2]	=	0x1260;		// 2346C  beq.s   $23480
	rom[0x23822/2]	=	0x1260;		// 23822  beq.s   $23822
	rom[0x2BCFA/2]	=	0x1260;		// 2BCFA  beq.s   $2BD0E
	rom[0x2C07E/2]	=	0x1260;		// 2C07E  beq.s   $2C092
	rom[0x2C434/2]	=	0x1260;		// 2C434  beq.s   $2C448
#endif
...
static void draw_layer_linescroll()
{
	...
	for (int y = 0; y < nScreenHeight; y++, dest += nScreenWidth)
	{
		...
		for (int x = 0; x < nScreenWidth + 16; x+=16) {
			...
			int xscroll = (scrollx[1] + swapWord(sram[y * 2 + 1]) + x) & 0x7ff;

			int ofst = (((yscroll >> 4) << 7) | (xscroll >> 4)) << 1;

			int attr = swapWord(vram[ofst]);
			int code = swapWord(vram[ofst + 1]) & 0x7fff;
...
static void draw_layer(unsigned char *ram, unsigned char *gfx, int size, int wide, int high, int color_offset, int color_and, int layer)
{
	...
	for (int sy = 0, offs = 0; sy < high * size; sy+=size)
	{
		...
		{
			int code = swapWord(vram[offs * 2 + 1]) & code_and;
			if (DrvTransTable[layer][code]) continue; // transparent

			int attr = swapWord(vram[offs * 2 + 0]);
...
static void draw_sprites()
{
	...
	for (int offs = 0; offs < 0x800 / 2; offs += 4)
	{
		...
		int attr = swapWord(sprite[offs + 1]);
		if (attr & 0x0100) break;

		int sy    = swapWord(sprite[offs + 0]);
		int code  = swapWord(sprite[offs + 2]);
		int sx    = swapWord(sprite[offs + 3]);
		int flipx = (attr & 0x8000);
		int flipy = (attr & 0x4000);
		int pri   = (attr & 0x0e00) >> 9;
...
static void copy_layers()
{
	...
	for (int y = 0; y < nScreenHeight; y++)
	{
		for (int x = 0; x < nScreenWidth; x++)
		{
			...
			*dest++ = pens[pri2layer[swapWord(pri_ram[pri_addr & 0x7fff]) & 7]] & 0x7ff;
		}
	}
}
...
struct BurnDriver BurnDrvLordgun = {
	"lordgun", NULL, NULL, "1994",
	"Lord of Gun (USA)\0", "Imperfect graphics", "IGS", "Miscellaneous",
	NULL, NULL, NULL, NULL,
	BDF_GAME_WORKING, 2, HARDWARE_MISC_MISC,
	NULL, lordgunRomInfo, lordgunRomName, LordgunInputInfo, LordgunDIPInfo,
	lordgunInit, DrvExit, lordgunFrame, lordgunDraw, DrvScan, &DrvRecalc,
	448, 224, 4, 3
};
...
struct BurnDriver BurnDrvAliencha = {
	"aliencha", NULL, NULL, "1994",
	"Alien Challenge (World)\0", NULL, "IGS", "Miscellaneous",
	NULL, NULL, NULL, NULL,
	BDF_GAME_WORKING, 2, HARDWARE_MISC_MISC,
	NULL, alienchaRomInfo, alienchaRomName, AlienchaInputInfo, AlienchaDIPInfo,
	alienchaInit, DrvExit, alienchaFrame, DrvDraw, DrvScan, &DrvRecalc,
	448, 224, 4, 3
};
...
struct BurnDriver BurnDrvAlienchac = {
	"alienchac", "aliencha", NULL, "1994",
	"Alien Challenge (China)\0", NULL, "IGS", "Miscellaneous",
	NULL, NULL, NULL, NULL,
	BDF_GAME_WORKING | BDF_CLONE, 2, HARDWARE_MISC_MISC,
	NULL, alienchacRomInfo, alienchacRomName, AlienchaInputInfo, AlienchacDIPInfo,
	alienchacInit, DrvExit, alienchaFrame, DrvDraw, DrvScan, &DrvRecalc,
	448, 224, 4, 3
};
