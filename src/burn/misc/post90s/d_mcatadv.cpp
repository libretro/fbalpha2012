...
static void draw_sprites(int priority)
{
	...
	while (source < finish)
	{
		...
		int tileno =  swapWord(source[1]);
		int x      =  swapWord(source[2]) & 0x03ff;
		int y      =  swapWord(source[3]) & 0x03ff;
		int flipy  =  attr & 0x0040;
		int flipx  =  attr & 0x0080;

		int height = (swapWord(source[3]) & 0xf000) >> 8;
		int width  = (swapWord(source[2]) & 0xf000) >> 8;
		int offset = tileno << 8;
		...
		if (swapWord(source[3]) != swapWord(source[0]))
		{
...
static void draw_background(unsigned char *vidramsrc, unsigned char *gfxbase, unsigned short *scroll, int priority, int max_tile)
{
	...
	if (~scroll[1] & 0x4000 && ~scroll[0] & 0x4000) { // row by row
		...
		for (int y = 0; y < 239; y+=16)
		{
			for (int x = 0; x < 335; x+=16)
			{
				...
				if ((swapWord(vidram[offs]) >> 14) != priority) continue;

				int code  = swapWord(vidram[offs | 1]);
				if (!code || code >= max_tile) continue;
				int color = ((swapWord(vidram[offs]) >> 8) & 0x3f) | ((scroll[2] & 3) << 6);
	...
	for (int y = 0; y < 224; y++, dest += 320) // line by line
	{
		...
		if (scroll[1] & 0x4000) scrolly  = swapWord(vidram[0x0800 + (scrolly * 2) + 1]) & 0x1ff;
		if (scroll[0] & 0x4000)	scrollx += swapWord(vidram[0x0800 + (scrolly * 2) + 0]);
		...
		for (int x = 0; x < 336; x+=16)
		{
			...
			int color = ((swapWord(vidram[offs]) >> 4) & 0x3f0) | ((scroll[2] & 3) << 10);
...
struct BurnDriver BurnDrvNost = {
	...
	NostInit, DrvExit, DrvFrame, DrvDraw, DrvScan, &DrvRecalc,
	224, 320, 3, 4
};
