...
static inline void palette_write(int offs)
{
...
static void draw_sprites()
{
	...
	{
		int zoom	= swapWord(s[0x000]);
		int sy		= swapWord(s[0x200]);
		int sx		= swapWord(s[0x400]);
		...
		{
			int code = swapWord(codes[y * 2]);
			if (code & 0x8000) continue; // only 8mb of sprites...

			int attr = swapWord(codes[y * 2 + 1]);
