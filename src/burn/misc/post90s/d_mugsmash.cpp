...
void __fastcall mugsmash_write_word(unsigned int address, unsigned short data)
{
	...
	switch (address)
	{
		...
		case 0x140004:
		case 0x140006:
			*((unsigned short*)(DrvSndRegs + (address & 7))) = swapWord(data);
		return;
	}
}
...
static void draw_sprites()
{
	...
	while (source < finish)
	{
		int xpos  = swapWord(source[0]) & 0x00ff;
		int ypos  = swapWord(source[4]) & 0x00ff;
		int num   = (swapWord(source[3]) & 0x00ff) | ((swapWord(source[2]) & 0x00ff) << 8);
		int attr  = swapWord(source[1]);
...
static void draw_layer(unsigned char *source, int colofst, int scroll)
{
	...
	for (int offs = 0; offs < 0x400; offs++)
	{
		...
		int code  = swapWord(vram[(offs << 1) | 1]);
		int color = swapWord(vram[(offs << 1)]);
