...
static void draw_bitmap()
{
	...
	for (y = 0, count = 0; y < 256; y++)
	{
		for(x = 0; x < 128; x++, count++)
		{
			...
			unsigned swappedBitmap = swapWord(bitmap[count]);

			int ofst = ((y - 9) * nScreenWidth) + ((x << 2) - 12);

			pix = swappedBitmap >> 12;
			if (pix) pTransDraw[ofst    ] = pix | 0x300;

			pix = (swappedBitmap &0x0f00)>>8;
			if (pix) pTransDraw[ofst | 1] = pix | 0x300;

			pix = (swappedBitmap &0x00f0)>>4;
			if (pix) pTransDraw[ofst | 2] = pix | 0x300;

			pix = swappedBitmap & 0x000f;
...
static void DrawTiles(unsigned char *vidram, int color, int transp, int scrollx, int scrolly)
{
	...
	for (int offs = 0; offs < 0x2000 / 2; offs++)
	{
		...
		int code  = swapWord(vram[offs]) | (gfx_bank << 16);
...
static void draw_sprites()
{
	...
	{
		int sx     = swapWord(spriteram16[offs + 2]) & 0x1ff;
		int sy     = swapWord(spriteram16[offs]);
		int code   = swapWord(spriteram16[offs + 1]);
		int color  = (swapWord(spriteram16[offs + 2]) >> 9) & 0xf;
