...
#define STRANGE_RAM_WRITE_WORD(value)						\
	if ((address & 0xffff0000) == value) {					\
		*((unsigned short*)(Drv68KRAM + (address & 0xfffe))) = swapWord(data);	\
		return;								\
	}									\
...
#else
			if(nmk16_mainram[0xef00/2] == 0xfe60)
			{
				nmk16_mainram[0xef00/2] = 0x0000; // this is the coin counter
				nmk16_mainram[0xef02/2] = 0x0000;
				nmk16_mainram[0xef04/2] = 0xf94e;
				nmk16_mainram[0xef06/2] = 0x0000;
				nmk16_mainram[0xef08/2] = 0xf492;
			}
#endif
		break;
	}
...
static inline int DrvSynchroniseStream(int nSoundRate)
{
	return (long long)(ZetTotalCycles() * nSoundRate / 3000000);
}

static inline double DrvGetTime()
{
	return (double)ZetTotalCycles() / 3000000;
}

static inline int Macross2SynchroniseStream(int nSoundRate)
{
	return (long long)(ZetTotalCycles() * nSoundRate / 4000000);
}

static inline double Macross2GetTime()
{
...
static inline double NMK004GetTime()
{
	return (double)SekTotalCycles() / nNMK004CpuSpeed;
}

static inline int NMK004SynchroniseStream(int nSoundRate)
{
...
static void draw_sprites(int flip, int coloff, int coland, int priority)
{
	...
	{
		if (swapWord(sprram[offs]) & 0x0001)
		{
			int sx    =  (swapWord(sprram[offs+4]) & 0x01ff) + videoshift;
			int sy    =  (swapWord(sprram[offs+6]) & 0x01ff);
			int code  =   swapWord(sprram[offs+3]) & nGraphicsMask[2];
			int color =   swapWord(sprram[offs+7]) & coland;
			int w     =  (swapWord(sprram[offs+1]) & 0x000f);
			int h     = ((swapWord(sprram[offs+1]) & 0x00f0) >> 4);
			int pri   = ((swapWord(sprram[offs+0]) & 0x00c0) >> 6);
			int flipy = ((swapWord(sprram[offs+1]) & 0x0200) >> 9);
			int flipx = ((swapWord(sprram[offs+1]) & 0x0100) >> 8);
...
static void draw_macross_background(unsigned char *ram, int scrollx, int scrolly, int coloff, int t)
{
	...
	for (int offs = 0; offs < 0x100 * 0x20; offs++)
	{
		...
		if (is_8bpp) {
			int code = swapWord(vram[ofst]) & nGraphicsMask[1];

			Render16x16Tile_Clip(pTransDraw, code, sx, sy, 0, 8, coloff, DrvGfxROM1);
		} else {
			int code  = ((swapWord(vram[ofst]) & 0xfff) | (*tilebank << 12)) & nGraphicsMask[1];
			int color =  swapWord(vram[ofst]) >> 12;
...
static void draw_gunnail_background(unsigned char *ram)
{
	...
	{
		int yscroll = (swapWord(scroll[0x100]) + swapWord(scroll[0x100 | y]) + y) & 0x1ff;

		int row = yscroll >> 4;
		int yl = (yscroll & 0x0f) << 4;

		int ofst0 = ((row >> 4) << 12) | (row & 0x0f);
		int xscroll0 = swapWord(scroll[0]) + swapWord(scroll[y]) - videoshift;
		...
		for (int x = 0; x < nScreenWidth + 16; x+=16)
		{
			...
			int code  = (swapWord(vram[ofst]) & 0xfff) | bank;
			int color =  (swapWord(vram[ofst]) >> 12) << 4;
...
static void bioship_draw_background(int scrollx, int scrolly)
{
	...
	for (int offs = 0; offs < 0x1000; offs++)
	{
		...
		int code = swapWord(tilerom[offs | bank]);
...
{
	unsigned short *scroll = (unsigned short*)DrvScrollRAM;
	int scrollx = swapWord(scroll[0x82 / 2]) & 0xfff;
	int scrolly = swapWord(scroll[0xc2 / 2]) & 0x1ff;
...
{
	unsigned short *scroll = (unsigned short*)DrvScrollRAM;
	int scrollx = ((swapWord(scroll[0]) & 0x0f) << 8) | (swapWord(scroll[1]) & 0xff);
	int scrolly = ((swapWord(scroll[2]) & 0x01) << 8) | (swapWord(scroll[3]) & 0xff);
...
{
	unsigned short *scroll = (unsigned short*)DrvScrollRAM;
	int scrollx = ((swapWord(scroll[0]) & 0x0f) << 8) | (swapWord(scroll[1]) >> 8);
	int scrolly = ((swapWord(scroll[2]) & 0x01) << 8) | (swapWord(scroll[3]) >> 8);
...
{
	unsigned short *scroll = (unsigned short*)DrvScrollRAM;
	int scrollx = swapWord(scroll[2]) & 0xff;
	int scrolly = swapWord(scroll[3]) & 0xff;
...
{
	unsigned short *scroll = (unsigned short*)DrvScrollRAM;
	int scrolly = (swapWord(scroll[3]) + 0x100) & 0x1ff;
	int scrollx = (swapWord(scroll[2]) + 0x000) & 0xfff;
...
static int StrahlDraw()
{
	...
	int bgscrollx = ((swapWord(scroll[0x000]) & 0x0f) << 8) | (swapWord(scroll[0x001]) & 0xff);
	int bgscrolly = ((swapWord(scroll[0x002]) & 0x01) << 8) | (swapWord(scroll[0x003]) & 0xff);
	int fgscrollx = ((swapWord(scroll[0x200]) & 0x0f) << 8) | (swapWord(scroll[0x201]) & 0xff);
	int fgscrolly = ((swapWord(scroll[0x202]) & 0x01) << 8) | (swapWord(scroll[0x203]) & 0xff);
...
static int GunnailDraw()
{
	...
	switch ((swapWord(scroll[0]) >> 8) & 0x30)
	{
...
static int BioshipDraw()
{
	...
	unsigned short *scroll = (unsigned short*)DrvScrollRAM;
	int bgscrolly = (swapWord(scroll[0x02]) & 0x100) | (swapWord(scroll[0x03]) >> 8);
	int bgscrollx = (swapWord(scroll[0x00]) & 0xf00) | (swapWord(scroll[0x01]) >> 8);
	int fgscrolly = (swapWord(scroll[0x02]) & 0x100) | (swapWord(scroll[0x03]) >> 8);
	int fgscrollx = (swapWord(scroll[0x00]) & 0xf00) | (swapWord(scroll[0x01]) >> 8);
...
{
	unsigned short *scroll = (unsigned short*)DrvScrollRAM;
	int bgscrollx = (swapWord(scroll[1]) + xoffset) & 0xfff;
	int bgscrolly = (swapWord(scroll[0]) + yoffset) & 0x1ff;
	int txscrollx = (swapWord(scroll[3]) & 0xff);
	int txscrolly = (swapWord(scroll[2]) & 0xff);
...
struct BurnDriver BurnDrvTharrier = {
	...
	TharrierInit, DrvExit, DrvFrame, TharrierDraw, NULL, NULL,
	224, 256, 3, 4
};
...
struct BurnDriver BurnDrvManybloc = {
	...
	ManyblocInit, DrvExit, DrvFrame, ManyblocDraw, NULL, NULL,
	240, 256, 3, 4
};
...
struct BurnDriver BurnDrvSsmissin = {
	...
	SsmissinInit, CommonExit, SsmissinFrame, MacrossDraw, NULL, NULL,
	224, 256, 3, 4
};
...
struct BurnDriver BurnDrvMacross2 = {
	...
	Macross2Init, DrvExit, Macross2Frame, Macross2Draw, NULL, NULL,
	384, 224, 4, 3
};
...
struct BurnDriver BurnDrvStagger1 = {
	...
	Stagger1Init, AfegaExit, AfegaFrame, AfegaDraw, NULL, NULL,
	224, 256, 3, 4
};
...
struct BurnDriver BurnDrvRedhawk = {
	...
	RedhawkInit, AfegaExit, AfegaFrame, AfegaDraw, NULL, NULL,
	224, 256, 3, 4
};
...
struct BurnDriver BurnDrvRedhawki = {
	...
	RedhawkiInit, AfegaExit, AfegaFrame, RedhawkiDraw, NULL, NULL,
	256, 224, 4, 3
};
...
struct BurnDriver BurnDrvRedhawkb = {
	...
	RedhawkbInit, AfegaExit, AfegaFrame, RedhawkbDraw, NULL, NULL,
	256, 224, 4, 3
};
...
struct BurnDriver BurnDrvGrdnstrm = {
	...
	GrdnstrmInit, AfegaExit, AfegaFrame, FirehawkDraw, NULL, NULL,
	256, 224, 4, 3
};
...
struct BurnDriver BurnDrvGrdnstrmk = {
	...
	GrdnstrmkInit, AfegaExit, AfegaFrame, AfegaDraw, NULL, NULL,
	224, 256, 3, 4
};
...
struct BurnDriver BurnDrvPopspops = {
	...
	PopspopsInit, AfegaExit, AfegaFrame, Bubl2000Draw, NULL, NULL,
	256, 224, 4, 3
};
...
struct BurnDriver BurnDrvBubl2000 = {
	...
	Bubl2000Init, AfegaExit, AfegaFrame, Bubl2000Draw, NULL, NULL,
	256, 224, 4, 3
};
...
struct BurnDriver BurnDrvMangchi = {
	...
	MangchiInit, AfegaExit, AfegaFrame, Bubl2000Draw, NULL, NULL,
	256, 224, 4, 3
};
...
struct BurnDriver BurnDrvFirehawk = {
	...
	FirehawkInit, AfegaExit, AfegaFrame, FirehawkDraw, NULL, NULL,
	256, 224, 4, 3
};
...
struct BurnDriver BurnDrvSpec2k = {
	...
	Spec2kInit, AfegaExit, AfegaFrame, FirehawkDraw, NULL, NULL,
	256, 224, 4, 3
};
...
struct BurnDriver BurnDrvTwinactn = {
	...
	TwinactnInit, CommonExit, SsmissinFrame, MacrossDraw, NULL, NULL,
	256, 224, 4, 3
};
...
struct BurnDriver BurnDrvSabotenb = {
	...
	SabotenbInit, BjtwinExit, BjtwinFrame, BjtwinDraw, NULL, NULL,
	384, 224, 4, 3
};
...
struct BurnDriver BurnDrvCactus = {
	...
	CactusInit, BjtwinExit, BjtwinFrame, BjtwinDraw, NULL, NULL,
	384, 224, 4, 3
};
...
struct BurnDriver BurnDrvBjtwin = {
	...
	BjtwinGameInit, BjtwinExit, BjtwinFrame, BjtwinDraw, NULL, NULL,
	224, 384, 3, 4
};
...
struct BurnDriver BurnDrvNouryoku = {
	...
	NouryokuGameInit, BjtwinExit, BjtwinFrame, BjtwinDraw, NULL, NULL,
	384, 224, 4, 3
};
...
struct BurnDriver BurnDrvMustang = {
	...
	MustangInit, NMK004Exit, NMK004Frame, MacrossDraw, NULL, NULL,
	256, 224, 4, 3
};
...
struct BurnDriver BurnDrvMustangb = {
	...
	MustangbInit, SeibuSoundExit, SeibuSoundFrame, MacrossDraw, NULL, NULL,
	256, 224, 4, 3
};
...
struct BurnDriver BurnDrvMustangb2 = {
	...
	Mustangb2Init, SeibuSoundExit, SeibuSoundFrame, MacrossDraw, NULL, NULL,
	256, 224, 4, 3
};
...
#else
		*((unsigned short*)(Drv68KROM + 0x048a)) = 0x714e;
		*((unsigned short*)(Drv68KROM + 0x04aa)) = 0x714e;
#endif
	}
...
struct BurnDriver BurnDrvTdragon = {
	...
	TdragonInit, NMK004Exit, NMK004Frame, HachamfDraw, NULL, NULL,
	224, 256, 3, 4
};
...
struct BurnDriver BurnDrvTdragonb = {
	...
	TdragonbInit, SeibuSoundExit, SeibuSoundFrame, MacrossDraw, NULL, NULL,
	224, 256, 3, 4
};
...
struct BurnDriver BurnDrvAcrobatm = {
	...
	AcrobatmInit, NMK004Exit, NMK004Frame, MacrossDraw, NULL, NULL,
	224, 256, 3, 4
};
...
struct BurnDriver BurnDrvMacross = {
	...
	MacrossInit, NMK004Exit, NMK004Frame, MacrossDraw, NULL, NULL,
	224, 256, 3, 4
};
...
struct BurnDriver BurnDrvGunnail = {
	...
	GunnailInit, NMK004Exit, NMK004Frame, GunnailDraw, NULL, NULL,
	224, 384, 3, 4
};
...
struct BurnDriver BurnDrvBlkheart = {
	...
	BlkheartInit, NMK004Exit, NMK004Frame, MacrossDraw, NULL, NULL,
	256, 224, 4, 3
};
...
struct BurnDriver BurnDrvVandyke = {
	...
	VandykeInit, NMK004Exit, NMK004Frame, VandykeDraw, NULL, NULL,
	224, 256, 3, 4
};
...
struct BurnDriver BurnDrvVandykeb = {
	...
	VandykebInit, NMK004Exit, NMK004Frame, VandykeDraw, NULL, NULL,
	224, 256, 3, 4
};
...
struct BurnDriver BurnDrvHachamf = {
	...
	HachamfInit, NMK004Exit, NMK004Frame, HachamfDraw, NULL, NULL,
	256, 224, 4, 3
};
...
struct BurnDriver BurnDrvStrahl = {
	...
	StrahlInit, NMK004Exit, NMK004Frame, StrahlDraw, NULL, NULL,
	256, 224, 4, 3
};
...
struct BurnDriver BurnDrvBioship = {
	...
	BioshipInit, NMK004Exit, NMK004Frame, BioshipDraw, NULL, NULL,
	256, 224, 4, 3
};
...
struct BurnDriver BurnDrvRaphero = {
	...
	RapheroInit, NMK004Exit, NMK004Frame, RapheroDraw, NULL, NULL,
	224, 384, 3, 4
};
