...
static void TileForeground()
{
	for (int offs = 0; offs < 64*32; offs++) {
		...
		if ( x<0 || x>(320-8) || y<0 || y>(224-8)) continue;
			...
			if ((swapWord(RamFg[offs]) & 0x0FFF) == 0) continue;
			unsigned char *d = RomFg + (swapWord(RamFg[offs]) & 0x0FFF) * 32;
 			unsigned short c = ((swapWord(RamFg[offs]) & 0xF000) >> 8) | 0x200;
...
static void DrawSprites()
{
	...
	for ( ; source < finish; source += 8 ) {
		...
		if (!(swapWord(source[0])&1)) continue;

		int	size	=	swapWord(source[1]);
		int	code	=	(swapWord(source[3]) & 0x7fff) + ( (size & 0x0100) << 7 );
		int	sx		=	swapWord(source[4]);
		int	sy		=	swapWord(source[6]);
		int	color	=	swapWord(source[7]) & 0x3F;
...
struct BurnDriver BurnDrvPowerinsj = {
...
struct BurnDriver BurnDrvPowerinsa = {
...
struct BurnDriver BurnDrvPowerinsb = {
...
struct BurnDriver BurnDrvPowerinsjb = {
	"powerinsjb", "powerins", NULL, "1993",
	"Gouketsuji Ichizoku (Japan, bootleg)\0", NULL, "Atlus", "Miscellaneous",
	L"\u8C6A\u8840\u5BFA\u4E00\u65CF (Japan, bootleg)\0Gouketsuji Ichizoku\0", NULL, NULL, NULL,
	BDF_GAME_WORKING | BDF_CLONE | BDF_BOOTLEG, 2, HARDWARE_MISC_MISC,
	NULL, powerinbRomInfo, powerinbRomName, powerinsInputInfo, powerinsDIPInfo,
	powerinsInit, powerinsExit, powerinsFrame, NULL, powerinsScan, &bRecalcPalette,
	320, 224, 4, 3
