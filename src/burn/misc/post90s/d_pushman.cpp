...
static void draw_sprites()
{
	...
	for (int offs = 0x0800-4; offs >= 0; offs -= 4)
	{
		...
		int color = (swapWord(vram[offs + 1]) >> 2) & 0xf;
		int code  = swapWord(vram[offs + 0]) & 0x7ff;

		int flipx = swapWord(vram[offs + 1]) & 2;
		int flipy = swapWord(vram[offs + 1]) & 1;
...
struct BurnDriver BurnDrvBballs = {
	...
	bballsInit, DrvExit, DrvFrame, DrvDraw, DrvScan, &DrvRecalc,
	256, 224, 4, 3
};
