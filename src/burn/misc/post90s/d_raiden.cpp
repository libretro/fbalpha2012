...
static struct BurnRomInfo raidenuRomDesc[] = {
	...
	{ "9.u016",		    0x008000, 0x1922b25e, BRF_GRA }, 			// Tiles
	{ "10.u017",		0x008000, 0x5f90786a, BRF_GRA },
	{ "sei420.u011",	0x080000, 0xda151f0b, BRF_GRA },
	{ "sei430.u013",	0x080000, 0xac1f57ac, BRF_GRA },
	{ "sei440.u012",	0x080000, 0x946d7bde, BRF_GRA },
...
static int DrvInit()
{
	...
	if (game_drv == GAME_RAIDENA || game_drv == GAME_RAIDENT || game_drv == GAME_RAIDENU)
		seibu_sound_decrypt();
...
static void drawBackground()
{
	...
	for (offs = 0; offs < 32*32; offs++) {
		...
		if (x<=-16 || x>=256 || y<=-16 || y>=224) continue;
			...
			unsigned int tileno = swapWord(RamBg[offs]) & 0x0FFF;
			//if (tileno == 0) continue;
 			unsigned int c = (swapWord(RamBg[offs]) & 0xF000) >> 8;
...
static void drawForeground()
{
	...
	for (offs = 0; offs < 32*32; offs++) {
		...
		if (x<=-16 || x>=256 || y<=-16 || y>=224) continue;
			...
			unsigned int tileno = swapWord(RamFg[offs]) & 0x0FFF;
			if (tileno == 0) continue;
 			unsigned int c = (swapWord(RamFg[offs]) & 0xF000) >> 8;
 			unsigned short * p = (unsigned short *) pBurnDraw + y * 256 + x;
...
static void drawText()
{
	...
	for (offs = 0; offs < 32*32; offs++) {
		...
		if ( y<0 || y>=224) continue;
			...
			unsigned int tileno = (swapWord(RamTxt[offs]) & 0x00FF) | ((swapWord(RamTxt[offs]) & 0xC000) >> 6);
			if (tileno == 0) continue;
 			unsigned int c = (swapWord(RamTxt[offs]) & 0x0F00) >> 4;
 			unsigned short * p = (unsigned short *) pBurnDraw + y * 256 + x;
...
struct BurnDriver BurnDrvRaiden = {
	...
	NULL, NULL, NULL, NULL,
	BDF_GAME_WORKING | BDF_16BIT_ONLY | BDF_ORIENTATION_VERTICAL, 2, HARDWARE_MISC_MISC,
...
struct BurnDriver BurnDrvRaidena = {
	...
	NULL, NULL, NULL, NULL,
	BDF_GAME_WORKING | BDF_16BIT_ONLY | BDF_ORIENTATION_VERTICAL | BDF_CLONE, 2, HARDWARE_MISC_MISC,
...
struct BurnDriver BurnDrvRaidenu = {
	"raidenu", "raiden", NULL, "1990",
	"Raiden (US, SEI8904 + SEI9008 PCBs)\0", NULL, "Seibu Kaihatsu (Fabtek license)", "Miscellaneous",
	NULL, NULL, NULL, NULL,
	BDF_GAME_WORKING | BDF_16BIT_ONLY | BDF_ORIENTATION_VERTICAL | BDF_CLONE, 2, HARDWARE_MISC_MISC,
	NULL, raidenuRomInfo, raidenuRomName, raidenInputInfo, raidenDIPInfo,
	DrvInit, DrvExit, DrvFrameAlt, NULL, DrvScan, &bRecalcPalette,
	224, 256, 3, 4
};
