...
{
	unsigned short TCR	=	swapWord(RamTMP68301[(0x200 + i * 0x20)/2]);
	unsigned short MAX1	=	swapWord(RamTMP68301[(0x204 + i * 0x20)/2]);
	unsigned short MAX2	=	swapWord(RamTMP68301[(0x206 + i * 0x20)/2]);
...
{
	unsigned short TCR	= swapWord(RamTMP68301[(0x200 + i * 0x20)/2]);
	unsigned short IMR	= swapWord(RamTMP68301[0x94/2]);		// Interrupt Mask Register (IMR)
	unsigned short ICR	= swapWord(RamTMP68301[0x8e/2+i]);	// Interrupt Controller Register (ICR7..9)
	unsigned short IVNR	= swapWord(RamTMP68301[0x9a/2]);		// Interrupt Vector Number Register (IVNR)
...
static void tmp68301_update_irq_state(int i)
{
	...
	if	( !(IMR & (1<<i)) )	{
		unsigned short ICR = swapWord(RamTMP68301[0x80/2+i]);	// Interrupt Controller Register (ICR0..2)
...
{
	sekAddress &= 0x0003ff;
	RamTMP68301[ sekAddress >> 1 ] = swapWord(wordValue);
...
unsigned short __fastcall gundamexReadWord(unsigned int address)
{
	...
	if ((address & 0xfffc00) == 0xfffc00) {
		return swapWord(RamTMP68301[(address & 0x3ff) / 2]);
	}
...
static void DrvDraw()
{
	...
	if (swapWord(RamVReg[0x30/2]) & 1) { // Blank Screen
	...
	for ( ; s1 < end; s1+=4 ) {
		int num		= swapWord(s1[0]);
		int xoffs	= swapWord(s1[1]);
		int yoffs	= swapWord(s1[2]);
		int sprite	= swapWord(s1[3]);
		...
		for( ; num > 0; num--,s2+=4 ) {
			...
			if (sprite & 0x8000) {
				...
				int flipx;
				int flipy;
				int sx       = swapWord(s2[0]);
				int sy       = swapWord(s2[1]);
				int scrollx  = swapWord(s2[2]);
				int scrolly  = swapWord(s2[3]);
				...
				for (y = 0; y < (0x40 >> tilesize); y++)
				{
					...
					for (x = 0; x < 0x40;x++)
					{
						...
						attr  = swapWord(s3[0]);
						code  = swapWord(s3[1]) + ((attr & 0x0007) << 16);
						flipx = (attr & 0x0010);
						flipy = (attr & 0x0008);
						color = (attr & 0xffe0) >> 5;
				...
				int sx    = swapWord(s2[0]);
				int sy    = swapWord(s2[1]);
				int attr  = swapWord(s2[2]);
				int code  = swapWord(s2[3]) + ((attr & 0x0007) << 16);
		...
		if (swapWord(s1[0]) & 0x8000) break;	// end of list marker
	}
...
struct BurnDriver BurnDrvGundamex = {
	...
	gundamexInit, grdiansExit, grdiansFrame, NULL, grdiansScan, &bRecalcPalette,
	384, 224, 4, 3
};
...
struct BurnDriver BurnDrvDeerhunt = {
	...
	samshootInit, grdiansExit, samshootFrame, NULL, grdiansScan, &bRecalcPalette,
	320, 240, 4, 3
};
...
struct BurnDriverD BurnDrvFuncube2 = {
	...
	funcubeInit, grdiansExit, grdiansFrame, NULL, grdiansScan, &bRecalcPalette,
	320, 240, 4, 3
};
