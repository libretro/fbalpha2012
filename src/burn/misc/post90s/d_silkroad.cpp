...
static void draw_sprites(int pri)
{
	...
	while (maxspr < finish)
	{
		if ((swapLong(maxspr[1]) & 0xff000000) == 0xff000000) break;
		maxspr += 2;
	}
	...
	while (finish >= source)
	{
		int xpos   = (swapLong(finish[0]) & 0x01ff) - 50;
		int ypos   = (swapLong(finish[0]) >> 16)    - 16;
		int attr   =  swapLong(finish[1]) >> 16;
		int tileno = (swapLong(finish[1]) & 0xffff) | ((attr & 0x8000) << 1);
		finish -= 2;
...
static void draw_layer(int offset, int scrollx, int scrolly)
{
	...
	for (int offs = 0; offs < 0x4000 / 4; offs++)
	{
		...
		int code  =  swapLong(vidram[offs]) & 0xffff;
		int color = (swapLong(vidram[offs]) >> 16) & 0x1f;
		int flipx = (swapLong(vidram[offs]) >> 16) & 0x80;
