...
static void draw_sprites(unsigned short *sprites, unsigned char *gfx_base, int max_tile)
{
	...
	for (offs = 0xfc00/2; offs < 0x10000/2 ; offs += 4/2)
	{
		...
		int y		=	swapWord(sprites[ offs + 0 + 0x00000 / 2 ]);
		int x		=	swapWord(sprites[ offs + 1 + 0x00000 / 2 ]);
		int dim 	=	swapWord(sprites[ offs + 0 + 0x10000 / 2 ]);
		...
		for (dy = 0; dy < dimy * 8; dy += 8)
		{
			...
			for (dx = 0; dx < dimx * 8; dx += 8)
			{
				...
				int tile	=	swapWord(sprites[ addr + 0x00000 / 2 ]);
				int color	=	swapWord(sprites[ addr + 0x10000 / 2 ]);
...
struct BurnDriver BurnDrvBestbest = {
	...
	BestbestInit, DrvExit, BestbestFrame, DrvDraw, DrvScan,
...
struct BurnDriver BurnDrvSunaq = {
	...
	SunaqInit, DrvExit, SunaqFrame, DrvDraw, DrvScan,
	&DrvRecalc, 256, 224, 4, 3
};
...
struct BurnDriver BurnDrvUballoon = {
	...
	UballoonInit, DrvExit, UballoonFrame, DrvDraw, DrvScan,
	&DrvRecalc, 256, 224, 4, 3
};
...
struct BurnDriver BurnDrvBssoccer = {
	...
	BssoccerInit, DrvExit, BssoccerFrame, DrvDraw, DrvScan,
	&DrvRecalc, 256, 224, 4, 3
};
