...
static inline int JumppopSynchroniseStream(int nSoundRate)
{
...
static inline unsigned int HtchctchCalcCol(unsigned short nColour)
{
	int r, g, b;
	nColour = swapWord(nColour);
	r = pal5bit(nColour >>  0);
...
static inline unsigned int FncywldCalcCol(unsigned short nColour)
{
	int r, g, b;
	nColour = swapWord(nColour);
	r = pal4bit(nColour >> 8);
...
static inline unsigned int JumppopCalcCol(unsigned short nColour)
{
	int r, g, b;
	nColour = swapWord(nColour);
...
static void PangpangRenderPf2Layer()
{
	...
	for (my = 0; my < 32; my++) {
		for (mx = 0; mx < 64; mx++) {
			TileIndex = (mx & 0x1f) + ((my & 0x1f) << 5) + ((mx & 0x60) << 5);
			Attr = swapWord(VideoRam[TileIndex * 2 + 0]);
			Code = swapWord(VideoRam[TileIndex * 2 + 1]) & 0xfff;
			Code |= 0x1000;
			Colour = (Attr >> 12) & 0x0f;
...
static void FncywldRenderPf2Layer()
{
	...
	for (my = 0; my < 32; my++) {
		for (mx = 0; mx < 64; mx++) {
			TileIndex = (mx & 0x1f) + ((my & 0x1f) << 5) + ((mx & 0x60) << 5);
			Attr = swapWord(VideoRam[TileIndex * 2 + 1]);
			Code = swapWord(VideoRam[TileIndex * 2 + 0]);
			Colour = Attr & 0x1f;
...
static void JumppopRenderPf2Layer()
{
	...
	for (my = 0; my < 64; my++) {
		for (mx = 0; mx < 64; mx++) {
			Code = swapWord(VideoRam[TileIndex]);
			Code &= (DrvNumTiles - 1);
			Colour = 0;
...
static void PangpangRenderPf1Layer()
{
	...
	for (my = 0; my < 32; my++) {
		for (mx = 0; mx < 64; mx++) {
			TileIndex = (mx & 0x1f) + ((my & 0x1f) << 5) + ((mx & 0x60) << 5);
			Attr = swapWord(VideoRam[TileIndex * 2 + 0]);
			Code = swapWord(VideoRam[TileIndex * 2 + 1]);
			Colour = (Attr >> 12) & 0x0f;
...
static void JumppopRenderPf1Layer()
{
	...
	for (my = 0; my < 64; my++) {
		for (mx = 0; mx < 64; mx++) {
			Code = swapWord(VideoRam[TileIndex]) & 0x1fff;
			Code &= (DrvNumTiles - 1);
			Colour = 0;
...
static void PangpangRenderCharLayer()
{
	...
	for (my = 0; my < 32; my++) {
		for (mx = 0; mx < 64; mx++) {
			Attr = swapWord(VideoRam[TileIndex * 2 + 0]);
			Code = swapWord(VideoRam[TileIndex * 2 + 1]) & 0x1fff;
			Colour = (Attr >> 12) & 0x1f;
			Code &= (DrvNumChars - 1);
...
static void FncywldRenderCharLayer()
{
	...
	for (my = 0; my < 32; my++) {
		for (mx = 0; mx < 64; mx++) {
			Attr = swapWord(VideoRam[TileIndex * 2 + 1]);
			Code = swapWord(VideoRam[TileIndex * 2 + 0]) & 0x1fff;
			if (Code) {
				Colour = Attr & 0x1f;
				Code &= (DrvNumChars - 1);
...
static void DrvRenderSprites(int MaskColour, int xFlipped)
{
	...
	for (Offset = 0; Offset < DrvSpriteRamSize / 2; Offset += 4) {
		...
		Code = swapWord(SpriteRam[Offset + 1]) & DrvSpriteMask;
		if (!Code) continue;

		y = swapWord(SpriteRam[Offset]);
		Flash = y & 0x1000;
		if (Flash && (GetCurrentFrame() & 1)) continue;

		x = swapWord(SpriteRam[Offset + 2]);
