...
void __fastcall Zeropnt268KWriteWord(unsigned int a, unsigned short d)
{
	switch (a) {
		case 0x80010c: {
			...
			ScrollRam[0] = swapWord(d);
			return;
		}

		case 0x80010e: {
			UINT16* ScrollRam = (UINT16*)DrvScrollRam;
			ScrollRam[1] = swapWord(d);
			return;
		}

		case 0x800110: {
			UINT16* ScrollRam = (UINT16*)DrvScrollRam;
			ScrollRam[2] = swapWord(d);
			return;
		}

		case 0x800114: {
			UINT16* ScrollRam = (UINT16*)DrvScrollRam;
			ScrollRam[4] = swapWord(d);
			return;
		}

		case 0x800116: {
			UINT16* ScrollRam = (UINT16*)DrvScrollRam;
			ScrollRam[5] = swapWord(d);
			return;
		}
...
static void DrvRenderSprites(int PriorityDraw)
{
	for (int Offset = 0; Offset < 0x400; Offset += 4) {
		...
		int sx = swapWord(SpriteRam[Offset + 0]);
		int sy = swapWord(SpriteRam[Offset + 1]);
		int Code = swapWord(SpriteRam[Offset + 2]);
		int Attr = swapWord(SpriteRam[Offset + 3]);
...
static void Zeropnt2RenderSprites(int PriorityDraw)
{
	for (int Offset = 0; Offset < 0x200; Offset += 2) {
		...
		int sx = swapLong(SpriteRam[Offset + 0]) & 0xffff;
		int sy = swapLong(SpriteRam[Offset + 0]) >> 16;
		int Code = swapLong(SpriteRam[Offset + 1]) & 0xffff;
		int Attr = swapLong(SpriteRam[Offset + 1]) >> 16;
...
static void DrvRenderLayer(int Layer)
{
	...
	for (my = 0; my < 64; my++) {
		for (mx = 0; mx < 64; mx++) {
			Attr = swapWord(VideoRam[2 * TileIndex + 1]);
			Code = swapWord(VideoRam[2 * TileIndex + 0]);
			Colour = Attr & 0x1f;
			Flip = Attr >> 5;
			xFlip = (Flip >> 0) & 0x01;
			yFlip = (Flip >> 1) & 0x01;
...
static void Zeropnt2RenderLayer(int Layer)
{
	...
	UINT16 ScrollX = swapLong(ScrollRam[0]) & 0xffff;
	UINT16 ScrollY = swapLong(ScrollRam[0]) >> 16;

	if (Layer == 1) {
		ScrollX = swapLong(ScrollRam[2]) >> 16;
		ScrollY = swapLong(ScrollRam[5]) & 0xffff;
		VideoRam = (UINT32*)DrvVideo1Ram;
	}

	if (Layer == 2) {
		ScrollX = swapLong(ScrollRam[2]) & 0xffff;
		ScrollY = swapLong(ScrollRam[1]) & 0xffff;
	...
	for (my = 0; my < 64; my++) {
		for (mx = 0; mx < 64; mx++) {
			Code = swapLong(VideoRam[TileIndex]);
			Attr = Code >> 16;
			Code &= 0xffff;
			Colour = Attr & 0x1f;
			Flip = Attr >> 5;
...
static void DrvCalcPalette()
{
	...
	for (int i = 0; i < 0x4000; i += 2) {
		 Data1 = swapWord(PaletteRam[i & ~1]);
		 Data2 = swapWord(PaletteRam[i |  1]);
