...
unsigned short __fastcall vmetal_read_word(unsigned int address)
{
	...
	if ((address & 0xffff0000) == 0x160000) {
		...
		int offset = ((address & 0xfffe) | ((swapWord(vreg[0x0ab/2]) & 0x7f) << 16)) * 2;
...
static void draw_layer_8x8()
{
	...
	for (int y = 0; y < 224 / 8; y++) {
		for (int x = 0; x < 320 / 8; x++) {
			...
			unsigned int lut = (swapWord(vlut[index]) << 16) | (swapWord(vlut[index + 1]));
...
static void draw_layer_16x16(unsigned char *ram, int scrolloff)
{
	...
	for (int offs = 0; offs < 256 * 256; offs++)
	{
		...
		unsigned int lu = (swapWord(vlut[index]) << 16) | (swapWord(vlut[index + 1]) << 0);
...
static void draw_sprites() // metro_draw_sprites
{
	...
	int sprites     = swapWord(videoregs[0x00]) & 0x1ff;
	if (sprites == 0) return;

	int color_start = (swapWord(videoregs[0x04]) & 0x0f) << 4;
	...
	{
		if (swapWord(videoregs[0x02/2]) & 0x8000)
		{
			src = spriteram;
			inc = (8 / 2);
		} else {
		...
		for (int j = 0; j < sprites; j++)
		{
			...
			int pri = (swapWord(videoregs[0x01]) & 0x0300) >> 8;

			if (!(swapWord(videoregs[0x01]) & 0x8000))
			{
				if (curr_pri > (swapWord(videoregs[0x01]) & 0x001f))
					pri = (swapWord(videoregs[0x01]) & 0x0c00) >> 10;
			}

			int y      = swapWord(src[1]);
			int attr   = swapWord(src[2]);
			int code   = swapWord(src[3]);
...
struct BurnDriver BurnDrvVmetal = {
	...
	NULL, vmetalRomInfo, vmetalRomName, vmetalInputInfo, vmetalDIPInfo,
...
struct BurnDriver BurnDrvVmetaln = {
	...
	NULL, vmetalnRomInfo, vmetalnRomName, vmetalInputInfo, vmetalDIPInfo,
	DrvInit, DrvExit, DrvFrame, DrvDraw, DrvScan, &DrvRecalc,
	224, 320, 3, 4
};
