...
void __fastcall Wwfwfest68KWriteByte(unsigned int a, unsigned char d)
{
	if (a >= 0x0c0000 && a <= 0x0c1fff) {
		...
		CharRam[(a - 0x0c0000) >> 1] = swapWord(d);
		return;
	}
...
static void DrvRenderBg0Layer(int Opaque)
{
	...
	for (my = 0; my < 32; my++) {
		for (mx = 0; mx < 32; mx++) {
			...
			Code = swapWord(TileBase[1]) & 0xfff;
			Colour = swapWord(TileBase[0]) & 0x0f;
			Flip = (swapWord(TileBase[0]) & 0xc0) >> 6;
			xFlip = (Flip >> 0) & 0x01;
			yFlip = (Flip >> 1) & 0x01;
...
static void DrvRenderBg1Layer(int Opaque)
{
	...
	for (my = 0; my < 32; my++) {
		for (mx = 0; mx < 32; mx++) {
			...
			Code = swapWord(TileBase[0]) & 0xfff;
			Colour = (swapWord(TileBase[0]) & 0xf000) >> 12;
...
static void DrvRenderSprites()
{
	...
	while (Source < Finish) {
		...
		Enable = swapWord(Source[1]) & 1;

		if (Enable) {
			x = +(swapWord(Source[5]) & 0x00ff) | (swapWord(Source[1]) & 0x0004) << 6;
			if (x > 512 - 16) x -= 512;
			x += DrvSpriteXOffset;
			y = (swapWord(Source[0]) & 0x00ff) | (swapWord(Source[1]) & 0x0002) << 7;
			y = (256 - y) & 0x1ff;
			y -= 16;
			y -= 8;
			xFlip = (swapWord(Source[1]) & 0x0010) >> 4;
			yFlip = (swapWord(Source[1]) & 0x0008) >> 3;
			Chain = (swapWord(Source[1]) & 0x00e0) >> 5;
			Chain += 1;
			Code = (swapWord(Source[2]) & 0x00ff) | (swapWord(Source[3]) & 0x00ff) << 8;
			Colour = (swapWord(Source[4]) & 0x000f);
...
static void DrvRenderCharLayer()
{
	...
	for (my = 0; my < 32; my++) {
		for (mx = 0; mx < 64; mx++) {
			TileBase = &VideoRam[TileIndex * 2];
			Code = (swapWord(TileBase[0]) & 0xff) | ((swapWord(TileBase[1]) & 0x0f) << 8);
			Colour = (swapWord(TileBase[1]) & 0xf0) >> 4;
