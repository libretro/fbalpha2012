...
static void draw_layer(int scr, int pri)
{
	...
	for (int offs = 0; offs < 64 * 64; offs++)
	{
		...
		int code = swapWord(vram[ofst * 2 + 0]) & 0x3fff;
		int attr = swapWord(vram[ofst * 2 + 1]);
...
static void draw_sprites(int pri)
{
	...
	{
		int sx		= swapWord(vram[offs + 0]) - x_offset;
		int sy		= swapWord(vram[offs + 1]) - 0x0f;
		int code	= swapWord(vram[offs + 2]) & 0x1fff;
		int attr	= swapWord(vram[offs + 3]);
		int flipx	= attr & 0x20;
		int flipy	= attr & 0x40;
...
struct BurnDriver BurnDrvMagicbub = {
	...
	MagicbubInit, DrvExit, DrvFrame, DrvDraw, DrvScan, &DrvRecalc,
	320, 224, 4, 3
};
...
struct BurnDriver BurnDrvMagicbuba = {
	...
	MagicbubaInit, DrvExit, DrvFrame, DrvDraw, DrvScan, &DrvRecalc,
	320, 224, 4, 3
};
...
struct BurnDriver BurnDrvPaprazzi = {
	...
	PaprazziInit, DrvExit, DrvFrame, DrvDraw, DrvScan, &DrvRecalc,
	224, 384, 3, 4
};
...
struct BurnDriver BurnDrvShocking = {
	...
	ShockingInit, DrvExit, DrvFrame, DrvDraw, DrvScan, &DrvRecalc,
	384, 224, 4, 3
};
