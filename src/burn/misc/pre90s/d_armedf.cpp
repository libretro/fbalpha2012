...
static void draw_layer(unsigned char *ram, unsigned char *gfxbase, int scrollx, int scrolly, int coloff, int code_and)
{
	...
	for (int offs = 0; offs < 64 * 32; offs++)
	{
		...
		int code = swapWord(vram[offs]) & code_and;
		int color = swapWord(vram[offs]) >> 11;
...
static void draw_txt_layer(int transp)
{
	...
	for (int offs = 0; offs < 64 * 32; offs++)
	{
		...
		int attr = swapWord(vram[ofst+ofsta]) & 0xff;
		int code = (swapWord(vram[ofst]) & 0xff) | ((attr & 3) << 8);
...
static void draw_sprites(int priority)
{
	...
	{
		int attr  = swapWord(spr[offs + 0]);
		if (((attr & 0x3000) >> 12) != priority) continue;

		int code  = swapWord(spr[offs + 1]);
		int flipx = code & 0x2000;
		int flipy = code & 0x1000;
		int color =(swapWord(spr[offs + 2]) >> 8) & 0x1f;
		int sx    = swapWord(spr[offs + 3]);
...
struct BurnDriver BurnDrvArmedf = {
	...
	ArmedfInit, DrvExit, DrvFrame, DrvDraw, DrvScan, &DrvRecalc,
	240, 320, 3, 4
};
...
struct BurnDriver BurnDrvCclimbr2 = {
	...
	Cclimbr2Init, DrvExit, DrvFrame, DrvDraw, DrvScan, &DrvRecalc,
	288, 224, 4, 3
};
...
static struct BurnRomInfo cclimbr2aRomDesc[] = {
...
STD_ROM_PICK(cclimbr2a)
STD_ROM_FN(cclimbr2a)

struct BurnDriver BurnDrvCclimbr2a = {
	...
	NULL, cclimbr2aRomInfo, cclimbr2aRomName, Cclimbr2InputInfo, Cclimbr2DIPInfo,
	Cclimbr2Init, DrvExit, DrvFrame, DrvDraw, DrvScan, &DrvRecalc,
	288, 224, 4, 3
};
...
struct BurnDriver BurnDrvKodure = {
	...
	KodureInit, DrvExit, DrvFrame, DrvDraw, DrvScan, &DrvRecalc,
	288, 224, 4, 3
};
...
static int LegionInit()
{
	...
	if (nRet == 0) { // hack
		*((unsigned short*)(Drv68KROM + 0x001d6)) = swapWord(0x0001);
		*((unsigned short*)(Drv68KROM + 0x00488)) = swapWord(0x4e71);
	}
...
struct BurnDriver BurnDrvLegion = {
	...
	LegionInit, DrvExit, DrvFrame, DrvDraw, DrvScan, &DrvRecalc,
	224, 288, 3, 4
};
...
struct BurnDriver BurnDrvLegiono = {
	...
	LegionoInit, DrvExit, DrvFrame, DrvDraw, DrvScan, &DrvRecalc,
	224, 288, 3, 4
};
...
struct BurnDriver BurnDrvTerraf = {
	...
	TerrafInit, DrvExit, DrvFrame, DrvDraw, DrvScan, &DrvRecalc,
	320, 240, 4, 3
};
...
struct BurnDriver BurnDrvTerrafu = {
	...
	TerrafuInit, DrvExit, DrvFrame, DrvDraw, DrvScan, &DrvRecalc,
	320, 240, 4, 3
};
...
struct BurnDriverD BurnDrvTerrafb = {
	...
	NULL, NULL, NULL, NULL,
	BDF_CLONE | BDF_BOOTLEG, 2, HARDWARE_MISC_MISC,
	NULL, terrafbRomInfo, terrafbRomName, ArmedfInputInfo, TerrafDIPInfo,
	TerrafbInit, DrvExit, DrvFrame, DrvDraw, DrvScan, &DrvRecalc,
	320, 240, 4, 3
};
