...
void __fastcall bionicc_write_word(unsigned int address, unsigned short data)
{
	...
	switch (address)
	{
		...
		case 0xfe801a:
			...
			inp[0] = swapWord((DrvInputs[0] >> 4) ^ 0x0f);
			inp[1] = swapWord(DrvInputs[2] ^ 0xff);
			inp[2] = swapWord(DrvInputs[1] ^ 0xff);
			return;
	}
}
...
static void draw_foreground(int priority)
{
	...
	for (int offs = 0; offs < 0x4000 / 4; offs++)
	{
		...
		int attr  = swapWord(vidram[(offs << 1) | 1]);
		int code  = (swapWord(vidram[offs << 1]) & 0xff) | ((attr & 0x07) << 8);
...
static int DrvDraw()
{
...

	{
		...
		{
			int code = swapWord(vidram[offs]) & 0x7ff;

			if (code != 0x7ff)
			{
				int attr = swapWord(vidram[offs+1]);
				int color = (attr&0x3C)>>2;
				int flipx = attr & 0x02;
				int sx = (short)swapWord(vidram[offs+3]);
				int sy = (short)swapWord(vidram[offs+2]);
				if(sy>512-16) sy-=512;
...

	{
		...
		for (int offs = 0; offs < 0x800 / 2; offs++)
		{
			...
			int attr  = swapWord(vidram[offs + 0x400]);
			int code  = (swapWord(vidram[offs]) & 0xff) | ((attr & 0xc0) << 2);
			int color = attr & 0x3f;
