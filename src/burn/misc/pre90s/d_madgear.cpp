...
static struct BurnRomInfo Ledstorm2RomDesc[] = {
...
STD_ROM_PICK(Ledstorm2)
STD_ROM_FN(Ledstorm2)
...
static void DrvRenderBgLayer()
{
	...
	for (mx = 0; mx < 32; mx++) {
		for (my = 0; my < 64; my++) {
			...
			Colour = swapWord(VideoRam[TileIndex + 0x800]);
			Flip = (Colour & 0x60) >> 5;
			xFlip = (Flip >> 0) & 0x01;
			yFlip = (Flip >> 1) & 0x01;
			Colour &= 0x0f;
...
static void DrvRenderSprites(int Priority)
{
	...
	for (Offset = 0x400 - 4; Offset >= 0; Offset -= 4) {
		...
		Attr = swapWord(SpriteRam[Offset + 1]);
		if (DrvSpritePriMask) {
			if (Priority == 1 && (Attr & DrvSpritePriMask)) continue;
			if (Priority == 0 && !(Attr & DrvSpritePriMask)) continue;
		}

		Code = swapWord(SpriteRam[Offset]) & 0xfff;
		sx = swapWord(SpriteRam[Offset + 3]) & 0x1ff;
		sy = swapWord(SpriteRam[Offset + 2]) & 0x1ff;
...
struct BurnDriver BurnDrvLastduelb = {
	"lastduelb", "lastduel", NULL, "1988",
	"Last Duel (bootleg)\0", NULL, "bootleg", "Miscellaneous",
	NULL, NULL, NULL, NULL,
	BDF_GAME_WORKING | BDF_CLONE | BDF_ORIENTATION_VERTICAL | BDF_BOOTLEG, 2, HARDWARE_MISC_MISC,
...
struct BurnDriver BurnDrvLedstorm2 = {
	"ledstorm2", "madgear", NULL, "1988",
	"Led Storm Rally 2011 (US)\0", NULL, "Capcom", "Miscellaneous",
	NULL, NULL, NULL, NULL,
	BDF_GAME_WORKING | BDF_CLONE | BDF_ORIENTATION_VERTICAL, 2, HARDWARE_MISC_MISC,
	NULL, Ledstorm2RomInfo, Ledstorm2RomName, DrvInputInfo, DrvDIPInfo,
