...
static void mrdo_palette_init()
{
	...
	for (int i = 0x0f; i >= 0; i--)
	{
		...
		if (i & 1) par += (float)1.0/150;
		if (i & 2) par += (float)1.0/120;
		if (i & 4) par += (float)1.0/100;
		if (i & 8) par += (float)1.0/75;
		if (par)
		{
			par = 1 / par;
...
struct BurnDriver BurnDrvmrdoy = {
	...
	NULL, NULL, NULL, NULL,
	BDF_GAME_WORKING | BDF_CLONE | BDF_PROTOTYPE | BDF_ORIENTATION_VERTICAL, 2, HARDWARE_MISC_MISC,
