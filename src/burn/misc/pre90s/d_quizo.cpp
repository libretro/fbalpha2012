...
static int DrvInit()
{
	...
	if (!strcmp(BurnDrvGetTextA(DRV_NAME), "quizo")) {
	...
	else if (!strcmp(BurnDrvGetTextA(DRV_NAME), "quizoa")) {
		if (BurnLoadRom(RomBank + 0x00000, 1, 1)) return 1;
		if (BurnLoadRom(RomBank + 0x04000, 2, 1)) return 1;
		if (BurnLoadRom(RomBank + 0x08000, 3, 1)) return 1;
		if (BurnLoadRom(RomBank + 0x0c000, 4, 1)) return 1;
		if (BurnLoadRom(RomBank + 0x10000, 5, 1)) return 1;
		if (BurnLoadRom(RomBank + 0x14000, 6, 1)) return 1;

		if (BurnLoadRom(Prom, 7, 1)) return 1;
	}
...
static struct BurnRomInfo quizoaRomDesc[] = {
	{ "7.bin",   0x4000, 0x1579ae31, BRF_ESS | BRF_PRG }, //  0 Z80 code

	{ "6.bin",   0x4000, 0xf00f6356, BRF_ESS | BRF_PRG }, //  1 Z80 code banks
	{ "5.bin",   0x4000, 0x39e577e3, BRF_ESS | BRF_PRG }, //  2
	{ "4.bin",   0x4000, 0xa977bd3a, BRF_ESS | BRF_PRG }, //  3
	{ "3.bin",   0x4000, 0x4411bcff, BRF_ESS | BRF_PRG }, //  4
	{ "2.bin",   0x4000, 0x4a0df776, BRF_ESS | BRF_PRG }, //  5
	{ "1.bin",   0x4000, 0xd9566c1a, BRF_ESS | BRF_PRG }, //  6

	{ "82s123",  0x0020, 0xc3f15914, BRF_GRA },	     //  7 Color Prom
};
...
struct BurnDriver BurnDrvQuizoa = {
	...
	NULL, quizoaRomInfo, quizoaRomName, DrvInputInfo, DrvDIPInfo,
	DrvInit, DrvExit, DrvFrame, NULL, NULL, NULL,
	320, 200, 4, 3
};
