...
{
	int r,g,b;
	unsigned short data = swapWord(*((unsigned short*)(DrvPalRam + (address & 0x0ffe))));
...
void __fastcall pow_write_word(unsigned int address, unsigned short data)
{
	if ((address & 0xffff8000) == 0x100000 && game_select & 1) {
		...
		*((unsigned short *)(DrvSprRam + (address & 0x7fff))) = swapWord(data);

		return;
	}

	if ((address & 0xfffff000) == 0x400000) {
		*((unsigned short *)(DrvPalRam + (address & 0x0ffe))) = swapWord(data);
...
static void sar_sprites(int j, int z, int pos)
{
	...
	for (offs = pos; offs < pos+0x800 ; offs += 0x80 )
	{
		mx=swapWord(spriteram16[(offs+j)>>1]);
		my=swapWord(spriteram16[(offs+j+2)>>1]);
		...
		for (i = 0; i < 0x80; i += 4)
		{
			color = swapWord(spriteram16[(offs+i+z)>>1]) & 0x7f;

			if (color)
			{
				tile = swapWord(spriteram16[(offs+2+i+z)>>1]);
...
static void pow_foreground()
{
	...
	for (int offs = 0; offs < 0x1000 / 2; offs+=2)
	{
		...
		int code  = (swapWord(vidram[offs | 0]) & 0xff) | pow_charbase;
		int color = swapWord(vidram[offs | 1]) & 0x0f;
...
struct BurnDriver BurnDrvsearchar = {
	...
	&DrvRecalc, 224, 256, 3, 4
};


// SAR - Search And Rescue (US)

static struct BurnRomInfo searcharuRomDesc[] = {
...
STD_ROM_PICK(searcharu)
STD_ROM_FN(searcharu)

struct BurnDriver BurnDrvsearcharu = {
	...
	NULL, searcharuRomInfo, searcharuRomName, IkariInputInfo, SarDIPInfo,
	searcharInit, DrvExit, DrvFrame, DrvDraw, DrvScan,
	&DrvRecalc, 224, 256, 3, 4
};
...
static struct BurnRomInfo searcharjRomDesc[] = {
...
STD_ROM_PICK(searcharj)
STD_ROM_FN(searcharj)

struct BurnDriver BurnDrvsearcharj = {
	"searcharj", "searchar", NULL, "1989",
	"SAR - Search And Rescue (Japan)\0", NULL, "SNK", "Miscellaneous",
	NULL, NULL, NULL, NULL,
	BDF_GAME_WORKING | BDF_CLONE | BDF_ORIENTATION_VERTICAL | BDF_ORIENTATION_FLIPPED, 2, HARDWARE_MISC_MISC,
	NULL, searcharjRomInfo, searcharjRomName, IkariInputInfo, SarDIPInfo,
	searcharInit, DrvExit, DrvFrame, DrvDraw, DrvScan,
	&DrvRecalc, 224, 256, 3, 4
};
...
struct BurnDriver BurnDrvstreetsm1 = {
	...
	NULL, NULL, NULL, NULL,
	BDF_GAME_WORKING | BDF_CLONE, 2, HARDWARE_MISC_MISC,
	NULL, streetsm1RomInfo, streetsm1RomName, DrvInputInfo, StreetsmDIPInfo,
	searcharInit, DrvExit, DrvFrame, DrvDraw, DrvScan,
	&DrvRecalc, 256, 224, 4, 3
};
...
struct BurnDriver BurnDrvstreetsmw = {
	...
	NULL, NULL, NULL, NULL,
	BDF_GAME_WORKING | BDF_CLONE, 2, HARDWARE_MISC_MISC,
	NULL, streetsmwRomInfo, streetsmwRomName, DrvInputInfo, StreetsmjDIPInfo,
	searcharInit, DrvExit, DrvFrame, DrvDraw, DrvScan,
	&DrvRecalc, 256, 224, 4, 3
};
...
struct BurnDriver BurnDrvstreetsmj = {
	...
	NULL, NULL, NULL, NULL,
	BDF_GAME_WORKING | BDF_CLONE, 2, HARDWARE_MISC_MISC,
	NULL, streetsmjRomInfo, streetsmjRomName, DrvInputInfo, StreetsmjDIPInfo,
	searcharInit, DrvExit, DrvFrame, DrvDraw, DrvScan,
	&DrvRecalc, 256, 224, 4, 3
};
