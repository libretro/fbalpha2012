...
{
	unsigned short data = swapWord(*((unsigned short*)(DrvPalRAM + offset)));
...
{
	if ((address & 0xff800) == 0x6e000) {
		*((unsigned short*)(DrvPalRAM + (address & 0x7fe))) = swapWord(data);
	...
	if (address >= 0x7180e && address <= 0x71e45) {
		if (is_bootleg)*((unsigned short*)(DrvSprRAM + (address & 0x7fe))) = swapWord(data);
		return;
	}

	if (address >= 0xa0000 && address <= 0xa0057) {
		if (!is_bootleg)*((unsigned short*)(DrvScrollRAM + (address & 0x3fe))) = swapWord(data);
		return;
	}
	...
	switch (address)
	{
		...
		case 0x75008:
		case 0x7500a:
			if (is_bootleg)*((unsigned short*)(DrvScrollRAM + (address - 0x75004))) = swapWord(data);
		return;
...
static void tokib_draw_sprites()
{
	...
	for (offs = 0;offs < 0x642 / 2;offs += 4)
	{
		...
		if (swapWord(sprite_word[0]) == 0xf100)
			break;

		if (swapWord(sprite_word[2]))
		{
			x = swapWord(sprite_word[3]) & 0x1ff;
			if (x > 256)
				x -= 512;

			y = swapWord(sprite_word[0]) & 0x1ff;
			if (y > 256)
				y = (512-y)+240;
			else
				y = 240-y;

			flipx   = swapWord(sprite_word[1]) & 0x4000;
			code    = swapWord(sprite_word[1]) & 0x1fff;
			color   = swapWord(sprite_word[2]) >> 12;
...
static void toki_draw_sprites()
{
	...
	for (offs = (0x800/2)-4;offs >= 0;offs -= 4)
	{
		...
		if ((swapWord(sprite_word[2]) != 0xf000) && (swapWord(sprite_word[0]) != 0xffff))
		{
			xoffs = (swapWord(sprite_word[0]) &0xf0);
			x = (swapWord(sprite_word[2]) + xoffs) & 0x1ff;
			if (x > 256)
				x -= 512;

			yoffs = (swapWord(sprite_word[0]) &0xf) << 4;
			y = (swapWord(sprite_word[3]) + yoffs) & 0x1ff;
			if (y > 256)
				y -= 512;

			color = swapWord(sprite_word[1]) >> 12;
			flipx   = swapWord(sprite_word[0]) & 0x100;
			flipy   = 0;
			code    = (swapWord(sprite_word[1]) & 0xfff) + ((swapWord(sprite_word[2]) & 0x8000) >> 3);
...
static int TokibDraw()
{
	...
	if (swapWord(scrollram[3]) & 0x2000) {
		draw_bg_layer(DrvBg1RAM, DrvGfxROM2, 0, 0x200, swapWord(scrollram[1])-0x103, swapWord(scrollram[0])+1+16);
		draw_bg_layer(DrvBg2RAM, DrvGfxROM3, 1, 0x300, swapWord(scrollram[3])-0x101, swapWord(scrollram[2])+1+16);
	} else {
		draw_bg_layer(DrvBg2RAM, DrvGfxROM3, 0, 0x300, swapWord(scrollram[3])-0x101, swapWord(scrollram[2])+1+16);
		draw_bg_layer(DrvBg1RAM, DrvGfxROM2, 1, 0x200, swapWord(scrollram[1])-0x103, swapWord(scrollram[0])+1+16);
	}
...
static int DrvDraw()
{
	...
	bgscrollx = ((swapWord(scrollram[0x06]) & 0x7f) << 1) | ((swapWord(scrollram[0x06]) & 0x80) >> 7) | ((swapWord(scrollram[0x05]) & 0x10) << 4);
	bgscrolly = ((swapWord(scrollram[0x0e]) & 0x7f) << 1) | ((swapWord(scrollram[0x0e]) & 0x80) >> 7) | ((swapWord(scrollram[0x0d]) & 0x10) << 4);

	fgscrollx = ((swapWord(scrollram[0x16]) & 0x7f) << 1) | ((swapWord(scrollram[0x16]) & 0x80) >> 7) | ((swapWord(scrollram[0x15]) & 0x10) << 4);
	fgscrolly = ((swapWord(scrollram[0x1e]) & 0x7f) << 1) | ((swapWord(scrollram[0x1e]) & 0x80) >> 7) | ((swapWord(scrollram[0x1d]) & 0x10) << 4);

	if (~nBurnLayer & 1) memset (pTransDraw, 0, nScreenWidth * nScreenHeight * 2);

	if (swapWord(scrollram[0x28]) & 0x0100) {
...
struct BurnDriver BurnDrvJujub = {
	...
	JujubInit, DrvExit, DrvFrame, DrvDraw, NULL, NULL,
	256, 224, 4, 3
};
...
struct BurnDriver BurnDrvTokib = {
	...
	TokibInit, DrvExit, TokibFrame, TokibDraw, NULL, NULL,
	256, 224, 4, 3
};
