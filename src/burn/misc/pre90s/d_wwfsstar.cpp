...
static void draw_fg_layer()
{
	...
	for (int offs = 0; offs < 32 * 32; offs++)
	{
		...
		int code  = (swapWord(vram[(offs * 2) + 1]) & 0xff) | ((attr & 0x0f) << 8);
...
static void draw_bg_layer()
{
	...
	for (int offs = 0; offs < 32 * 32; offs++)
	{
		...
		int attr  = swapWord(vram[(ofst * 2) + 0]);
		int code  = (swapWord(vram[(ofst * 2) + 1]) & 0xff) | ((attr & 0x0f) << 8);
...
static void draw_sprites()
{
	...
	while (source < finish)
	{
		if (swapWord(source[1]) & 0x0001)
		{
			int ypos   = (swapWord(source[0]) & 0xff) | ((swapWord(source[1]) & 0x04) << 6);
			int xpos   = (swapWord(source[4]) & 0xff) | ((swapWord(source[1]) & 0x08) << 5);
			int flipx  = (swapWord(source[2]) & 0x80 ) >> 7;
			int flipy  = (swapWord(source[2]) & 0x40 ) >> 6;
			int chain  = (swapWord(source[1]) & 0x02 ) >> 1;
			int number = (swapWord(source[3]) & 0xff) | ((swapWord(source[2]) & 0x3f) << 8);
			int color  = ((swapWord(source[1]) & 0xf0) + 0x80) >> 4;
...
static void DrvPaletteRecalculate()
{
	...
	for (int i = 0; i < 0x180; i++) {
		r = (swapWord(p[i]) >> 0) & 0x0f;
		b = (swapWord(p[i]) >> 8) & 0x0f;
		g = (swapWord(p[i]) >> 4) & 0x0f;
