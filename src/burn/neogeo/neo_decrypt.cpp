...
static inline void decrypt(unsigned char *r0, unsigned char *r1,
...
void pcb_gfx_crypt(unsigned char* src, int nType)
{
	...
	for (i = 0; i < 0x800000 * 4; i+=4)
	{
		...
		*tmp = swapLong(BITSWAP32(swapLong(*tmp), 9,13,19,0,23,15,3,5,4,12,17,30,18,21,11,6,27,10,26,28,20,2,14,29,24,8,1,16,25,31,7,22));
	}
...
const unsigned int nNeoM1ROMSize = 0x80000;

void neogeo_cmc50_m1_decrypt()
{
	unsigned char *rom = NeoZ80ROM;
	unsigned char *buffer = (unsigned char*)malloc(nNeoM1ROMSize);

	unsigned short key = generate_cs16(rom, 0x10000);

	for (int i = 0; i < nNeoM1ROMSize; i++) {
		buffer[i] = rom[m1_address_scramble(i, key)];
	}

	memcpy(rom, buffer, nNeoM1ROMSize);
	free(buffer);
}
