...
static int NeoLoad68KBIOS(int nNewBIOS)
{
	...
	if (!strcmp(BurnDrvGetTextA(DRV_NAME), "kf2k3pcb") || !strcmp(BurnDrvGetTextA(DRV_NAME), "kf2k3pcd")) {
...
//	NeoSpriteROM = (unsigned char*)malloc(nSpriteSize);
	...
	// Allocate all memory is needed for ROM
	{
		...
		unsigned int nLen = ROMEnd - (unsigned char*)0;
	...
	if (pInfo->nADPCMANum) {
		...
		if ((!strcmp(BurnDrvGetTextA(DRV_NAME), "pbobblenb")) || (!strcmp(BurnDrvGetTextA(DRV_NAME), "pbobblcd"))) { // pbobble fix
	...
	// This is for saving decrypted M1 roms
	if (bsavedecryptedm1 == 1) {
		SaveDecM1ROM();
	}
...
static inline void MapPalette(int nBank)
{
...
{
	if (bSRAMWritable) {
		*((unsigned short*)(NeoNVRAM + (sekAddress & 0xffff))) = swapWord(wordValue);	
	}
}
...
int NeoInit()
{
	// ==> set the neo screen width, added by regret
	int dummy;
	BurnDrvGetVisibleSize(&nNeoScreenWidth, &dummy);
	// <==
	...
	// Allocate all memory is needed for RAM
	{
		...
		unsigned int nLen = RAMEnd - (unsigned char*)0;
...
static void NeoStandardInputs(int nBank)
{
	if (nBank) {
		...
		DrvClearOpposites(&NeoInput[8]);
		DrvClearOpposites(&NeoInput[9]);
	...
	else
	{
		...
		DrvClearOpposites(&NeoInput[0]);
		DrvClearOpposites(&NeoInput[1]);
