...
static void pgm_drawsprites()
{
	...
	while (finish > source)
	{
...
#else
		UINT16 source0, source1, source2, source3, source4;

		source0 = swapWord(source[0]);
		source1 = swapWord(source[1]);
		source2 = swapWord(source[2]);
		source3 = swapWord(source[3]);
		source4 = swapWord(source[4]);

		if (source4 == 0) break; // right?

		int xpos =  source0 & 0x07ff;
		int xzom = (source0 & 0x7800) >> 11;
		int xgrow= (source0 & 0x8000) >> 15;
		int ypos =  source1 & 0x03ff;
		int yzom = (source1 & 0x7800) >> 11;
		int ygrow= (source1 & 0x8000) >> 15;
		int boff =((source2 & 0x007f) << 16) | (source3 & 0xffff);
		int prio = (source2 & 0x0080) >> 7;
		int palt = (source2 & 0x1f00) >> 8;
		int flip = (source2 & 0x6000) >> 13;
		int high =  source4 & 0x01ff;
		int wide = (source4 & 0x7e00) >> 9;
		...
		draw_sprite_new_zoomed(wide, high, xpos, ypos, palt, boff << 1, flip, xzoom, xgrow, yzoom, ygrow, prio);
...
static void draw_text()
{
	...
	int scrollx = ((signed short)swapWord(PGMVidReg[0x6000 / 2])) & 0x1ff;
	int scrolly = ((signed short)swapWord(PGMVidReg[0x5000 / 2])) & 0x0ff;

	for (int offs = 0; offs < 64 * 32; offs++)
	{
		int code = swapWord(vram[offs * 2]);
		...
		int attr = swapWord(vram[offs * 2 + 1]);
...
static void draw_background()
{
	...
	int yscroll = (signed short)swapWord(PGMVidReg[0x2000 / 2]);
	int xscroll = (signed short)swapWord(PGMVidReg[0x3000 / 2]);
	...
	int t = 0;
	{
		...
		for (int i = 0; i < 224; i++) {
			if (swapWord(rs[0]) != swapWord(rs[i])) {
				t = 1;
				break;
			}
		}
	...
	if (t == 0)
	{
		...
		for (int offs = 0; offs < 64 * 16; offs++)
		{
			...
			int color = ((swapWord(vram[offs*2+1]) & 0x3e) >> 1) | 0x20;
			int flipy = swapWord(vram[offs*2+1]) & 0x80;
			int flipx = swapWord(vram[offs*2+1]) & 0x40;
	...
	{
		int scrollx = (xscroll + swapWord(rowscroll[y])) & 0x7ff;
