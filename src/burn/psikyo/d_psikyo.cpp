...
static struct BurnDIPInfo samuraiaDIPList[] = {
	...
	{0x15,	0x01, 0x0E,	0x06, "1 coin = 2 credits"},
	{0x15,	0x01, 0x0E,	0x08, "1 coin = 3 credits"},
	...
	{0x15,	0x00, 0xFF, 0xFF, NULL},
	{0x15,	0x82, 0x70,	0x30, "1 coin = 2 credits"},
	{0x15,	0x00, 0xFF, 0xFF, NULL},
	{0x15,	0x82, 0x70,	0x40, "1 coin = 3 credits"},
	{0x15,	0x00, 0xFF, 0xFF, NULL},
...
static struct BurnDIPInfo s1945DIPList[] = {
	...
	{0x15,	0x01, 0x70,	0x40, "1 coin = 3 credits"},
...
static void Nibbleswap(unsigned char* pData, int nLen)
{
	for (int i = nLen -2 ; i >= 0; i -= 2) {
		...
		((unsigned short*)pData)[i + 0] = swapWord((pData[i + 1] >> 4) | (pData[i + 1] << 8)) & 0x0F0F;
		((unsigned short*)pData)[i + 1] = swapWord(c);
	}
...
static void tengaiNibbleswap(unsigned char* pData, int nLen)
{
	for (int i = nLen - 1; i >= 0; i--) {
		((unsigned short*)pData)[i] = swapWord((pData[i] >> 4) | (pData[i] << 8)) & 0x0F0F;
	}
...
struct BurnDriver BurnDrvTengaij = {
	...
	BDF_GAME_WORKING | BDF_16BIT_ONLY | BDF_CLONE, 2, HARDWARE_PSIKYO,
