...
int PsikyoSpriteRender(int nLowPriority, int nHighPriority)
{
	...
	nRenderFunction = (swapWord(((unsigned short*)PsikyoSpriteRAM)[0x0FFF]) & 4) << 4;
		...
		if (pBuffer->xzoom == 0 && pBuffer->yzoom == 0) {						// This sprite doesn't use zooming
			...
			for (int y = 0; y < nYSize; y++, nTileYPos += 16, pStart += 16 * nBurnPitch, pZStart += 16 * 320) {
				...
				for (int x = 0; x < nXSize; x++, nTileXPos += 16, nAddress += nNextTileX, pTile += nBurnBpp << 4, pZTile += 16) {
					...
					if (PsikyoSpriteAttrib[swapWord(((unsigned short*)PsikyoSpriteLUT)[nAddress])] == nTransColour) {
						continue;
					}

					pTileData = PsikyoSpriteROM + ((swapWord(((unsigned short*)PsikyoSpriteLUT)[nAddress]) << 8) & nSpriteAddressMask);
...
int PsikyoSpriteBuffer()
{
        ...
        if (swapWord(((unsigned short*)PsikyoSpriteRAM)[0x0FFF]) & 1) {
                return 0;
        }

        for (int i = 0x0C00, z = 0; i < 0x0FFF; i++) {

                word = swapWord(((unsigned short*)PsikyoSpriteRAM)[i]);
                ...
                x = swapWord(pSprite[1]) & 0x01FF;
                y = swapWord(pSprite[0]) & 0x01FF;

                xs = ((swapWord(pSprite[1]) >> 9) & 0x0007) + 1;
                ys = ((swapWord(pSprite[0]) >> 9) & 0x0007) + 1;
                ...
                pBuffer->xzoom = swapWord(pSprite[1]) >> 12;
                pBuffer->yzoom = swapWord(pSprite[0]) >> 12;
                ...
                pBuffer->address = swapWord(pSprite[3]) | ((word & 1) << 16);
