...
{
	color <<= 4;
	unsigned int *pal = pPsikyoshPalSrc + color;
				 ...
				 int offsy, int alpha, int zoomx, int zoomy, int wide, int high, int z)
{
	...
	else
	{
		...
		{
			unsigned int *pal = pPsikyoshPalSrc + (color << 4);
...
static void draw_sprites(unsigned char req_pri)
{
	...
	unsigned short *list = (unsigned short *)src + (0x3800>>1);
	unsigned short listlen = (0x800>>1);
...
#else
		unsigned int listdat = list[listcntr];
#endif
...
#else
				psikyosh_drawgfxzoom(dpth, tnum, colr, flpx, flpy, xpos, ypos, alpha,
					(unsigned int)zoom_table[zoomx],(unsigned int)zoom_table[zoomy], wide, high, listcntr);
#endif
			}
		}
...
static void draw_layer(int layer, int bank, int alpha, int scrollx, int scrolly)
{
	...
	for (int offs = 0; offs < size * 32; offs++) {
		...
		unsigned int code  = (pPsikyoshBgRAM[((bank*0x800)>>2) + offs - (0x4000>>2)]);
...
static void draw_bglayer(int layer)
{
	...
	bank    = (pPsikyoshBgRAM[(0x17f0>>2) + offset + layer] & 0x000000ff);
	alpha   = (pPsikyoshBgRAM[(0x17f0>>2) + offset + layer] & 0x0000bf00) >> 8;
	scrollx = (pPsikyoshBgRAM[(0x13f0>>2) + offset + layer] & 0x000001ff);
	scrolly = (pPsikyoshBgRAM[(0x13f0>>2) + offset + layer] & 0x03ff0000) >> 16;
...
static void draw_bglayertext(int layer)
{
	...
	bank    = (pPsikyoshBgRAM[((scrollbank*0x800)>>2) + (0x0400>>2) - (0x4000>>2)] & 0x000000ff);
	alpha   = (pPsikyoshBgRAM[((scrollbank*0x800)>>2) + (0x0400>>2) - (0x4000>>2)] & 0x0000bf00) >> 8;
	scrollx = (pPsikyoshBgRAM[((scrollbank*0x800)>>2) - (0x4000>>2)           ] & 0x000001ff);
	scrolly = (pPsikyoshBgRAM[((scrollbank*0x800)>>2) - (0x4000>>2)           ] & 0x03ff0000) >> 16;

	draw_layer(layer, bank, alpha, scrollx, scrolly);

	bank    = (pPsikyoshBgRAM[((scrollbank*0x800)>>2) + (0x0400>>2) + (0x20>>2) - (0x4000>>2)] & 0x000000ff);
	alpha   = (pPsikyoshBgRAM[((scrollbank*0x800)>>2) + (0x0400>>2) + (0x20>>2) - (0x4000>>2)] & 0x0000bf00) >> 8;
	scrollx = (pPsikyoshBgRAM[((scrollbank*0x800)>>2) - (0x4000>>2) + (0x20>>2)           ] & 0x000001ff);
	scrolly = (pPsikyoshBgRAM[((scrollbank*0x800)>>2) - (0x4000>>2) + (0x20>>2)           ] & 0x03ff0000) >> 16;
...
static void draw_background(unsigned char req_pri)
{
	for (int i = 0; i < 3; i++)
	{
		...
		{
			case 0x0a: // Normal
				if((pPsikyoshBgRAM[(0x17f0>>2) + ((i*0x04)>>2)] >> 24) == req_pri)
					draw_bglayer(i);
				break;

			case 0x0b: // Alt / Normal
				if((pPsikyoshBgRAM[(0x1ff0>>2) + ((i*0x04)>>2)] >> 24) == req_pri)
					draw_bglayer(i);
				break;

			case 0x0c: // Using normal for now
			case 0x0d: // Using normal for now
				if((pPsikyoshBgRAM[((bgtype*0x800)>>2) + (0x400>>2) - (0x4000>>2)] >> 24) == req_pri)
					draw_bglayertext(i);
				break;
...
{
	unsigned int *lineblend = pPsikyoshBgRAM + (0x0400>>2);
	unsigned int *destline = DrvTmpDraw;
...
{
	{
		for (int i = 0; i < (0x5000>>2); i++) {
			pPsikyoshPalSrc[i] = pPsikyoshPalRAM[i] >> 8;
		}
	}
...
static void calculate_transtab()
{
	...
	XMemSet (DrvTransTab, 0xff, 0x18000);
...
void PsikyoshVideoInit(int gfx_max, int gfx_min)
{
	...
	// mult once before the render instead of a bunch of times inside it
