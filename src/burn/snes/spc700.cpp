...
unsigned short getspcpc() { return spc.pc; }
...
void dumpspcregs()
{
	...
	fwrite(spcram,65536,1,f);
	fclose(f);
	printf("SPC registers :\n");
	printf("YA=%04X X=%02X S=%02X PC=%04X\n",spc.ya.w,spc.x,spc.s,spc.pc);
	printf("IN  : %02X %02X %02X %02X\n",ram[0xF4],ram[0xF5],ram[0xF6],ram[0xF7]);
