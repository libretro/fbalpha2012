...
static inline unsigned int QzquestCalcCol(unsigned short nColour)
{
	int r, g, b;
	nColour = swapWord(nColour);
...
static void TaitoF2MakeSpriteList()
{
	...
	if (Area == 0x8000 && swapWord(SpriteRamBuffered[(0x8000 + 6) / 2]) == 0 && swapWord(SpriteRamBuffered[(0x8000 + 10) / 2]) == 0) 
	{
		Area = 0;
	}
	...
	for (Off = 0; Off < 0x4000; Off += 16) {
		...
		if (swapWord(SpriteRamBuffered[(Offs + 6) / 2]) & 0x8000) {
			Disabled = swapWord(SpriteRamBuffered[(Offs + 10) / 2]) & 0x1000;
			TaitoF2SpritesFlipScreen = swapWord(SpriteRamBuffered[(Offs + 10) / 2]) & 0x2000;

			xOffset = TaitoXOffset;
			if (TaitoF2SpritesFlipScreen) xOffset = -TaitoXOffset;

			if (Footchmp) {
				Area = 0x8000 * (swapWord(SpriteRamBuffered[(Offs + 6) / 2]) & 0x0001);
			} else {
				Area = 0x8000 * (swapWord(SpriteRamBuffered[(Offs + 10) / 2]) & 0x0001);
			}
		}

		if ((swapWord(SpriteRamBuffered[(Offs + 4) / 2]) & 0xf000) == 0xa000) {
			MasterScrollX = swapWord(SpriteRamBuffered[(Offs + 4) / 2]) & 0xfff;
			if (MasterScrollX >= 0x800) MasterScrollX -= 0x1000;

			MasterScrollY = swapWord(SpriteRamBuffered[(Offs + 6) / 2]) & 0xfff;
			if (MasterScrollY >= 0x800) MasterScrollY -= 0x1000;
		}

		if ((swapWord(SpriteRamBuffered[(Offs + 4) / 2]) & 0xf000) == 0x5000) {
			xScroll1 = swapWord(SpriteRamBuffered[(Offs + 4) / 2]) & 0xfff;
			if (xScroll1 >= 0x800) xScroll1 -= 0x1000;

			yScroll1 = swapWord(SpriteRamBuffered[(Offs + 6) / 2]) & 0xfff;
		...
		SpriteData = swapWord(SpriteRamBuffered[(Offs + 8) / 2]);

		SpriteCont = (SpriteData & 0xff00) >> 8;

		if ((SpriteCont & 0x08) != 0) {
			if (BigSprite == 0) {
				xLatch = swapWord(SpriteRamBuffered[(Offs + 4) / 2]) & 0xfff;
				yLatch = swapWord(SpriteRamBuffered[(Offs + 6) / 2]) & 0xfff;
				xNum = 0;
				yNum = 0;
				ZoomWord = swapWord(SpriteRamBuffered[(Offs + 2) / 2]);
		...
		if (BigSprite == 0 || (SpriteCont & 0xf0) == 0) {
			x = swapWord(SpriteRamBuffered[(Offs + 4) / 2]);
			...
			x &= 0xfff;
			y = swapWord(SpriteRamBuffered[(Offs + 6) / 2]) & 0xfff;
		...
		if (TaitoF2SpriteType == 0) {
			Code = swapWord(SpriteRamBuffered[Offs / 2]) & 0x1fff;
			i = (Code & 0x1c00) >> 10;
			Code = SpriteBank[i] + (Code & 0x3ff);
		}

		if (TaitoF2SpriteType == 1) {
			Code = swapWord(SpriteRamBuffered[Offs / 2]) & 0x3ff;
			i = (swapWord(SpriteExtension[(ExtOffs >> 4)]) & 0x3f) << 10;
			Code = (i | Code);
		}

		if (TaitoF2SpriteType == 2) {
			Code = swapWord(SpriteRamBuffered[Offs / 2]) & 0xff;
			i = (swapWord(SpriteExtension[(ExtOffs >> 4)]) & 0xff00);
			Code = (i | Code);
		}

		if (TaitoF2SpriteType == 3) {
			Code = swapWord(SpriteRamBuffered[Offs / 2]) & 0xff;
			i = (swapWord(SpriteExtension[ExtOffs >> 4]) & 0xff) << 8;
			Code = (i | Code);
		}
