...
static void TaitoXDrawBgSprites()
{
	...
	int Ctrl = swapWord(SpriteRam[0x300]);
	int Ctrl2 = swapWord(SpriteRam[0x301]);
	...
	int Upper = (swapWord(SpriteRam[0x302]) & 0xff) +	(swapWord(SpriteRam[0x303]) & 0xff) * 256;
	int Col0;
	switch (Ctrl & 0x0f) {
	...
	for (Col = 0; Col < NumCol; Col++) {
		x = swapWord(SpriteRam[(Col * 0x20 + 0x08 + 0x400) / 2]) & 0xff;
		y = swapWord(SpriteRam[(Col * 0x20 + 0x00 + 0x400) / 2]) & 0xff;

		for (Offs = 0; Offs < 0x20; Offs++) {
			Code = swapWord(src[((Col + Col0) & 0x0f) * 0x20 + Offs + 0x400]);
			Colour = swapWord(src[((Col + Col0) & 0x0f) * 0x20 + Offs + 0x600]);
...
static void TaitoXDrawSprites()
{
	...
	for (Offset = (0x400 - 2) / 2; Offset >= 0; Offset-- ) {
		Code = swapWord(src[Offset]);
		x = swapWord(src[Offset + 0x200]);
		y = swapWord(SpriteRam[Offset]) & 0xff;
		xFlip = Code & 0x8000;
		yFlip = Code & 0x4000;
