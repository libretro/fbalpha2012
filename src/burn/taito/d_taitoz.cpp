...
static void AquajackRenderSprites(int PriorityDraw)
{
	...
	for (Offset = 0x200 - 4; Offset >= 0; Offset -= 4) {
		...
		for (SpriteChunk = 0; SpriteChunk < 32; SpriteChunk++) {
			...
			Code = swapWord(SpriteMap[MapOffset + px + (py << 2)]);
	...
	for (Offset = 0; Offset < 0x400; Offset += 4) {
		...
		if ((xZoom - 1) & 0x40) {
			...
			for (SpriteChunk = 0; SpriteChunk < 64; SpriteChunk++) {
				...
				Code = swapWord(SpriteMap[MapOffset + px + (py << 3)]);
				Code &= (TaitoNumSpriteA - 1);
			...
			for (SpriteChunk = 0; SpriteChunk < 16; SpriteChunk++) {
				...
				Code = swapWord(SpriteMap[MapOffset + px + (py << 1)]);
				Code &= (TaitoNumSpriteB - 1);
...
{
#pragma omp section
		if (!(Disable & 0x02)) TC0100SCNRenderFgLayerHack(TaitoChars);
#pragma omp section
		if (!(Disable & 0x01)) TC0100SCNRenderBgLayerHack(TaitoChars);
}
	} 
	...
	if (!(Disable & 0x04)) TC0100SCNRenderCharLayerHack();
	BurnTransferCopy(TC0110PCRPalette);
}
...
struct BurnDriver BurnDrvNightstru = {
	...
	NULL, NULL, NULL, NULL,
	BDF_GAME_WORKING | BDF_CLONE, 2, HARDWARE_TAITO_TAITOZ,
	NULL, NightstruRomInfo, NightstruRomName, NightstrInputInfo, NightstrDIPInfo,
	NightstrInit, TaitoZExit, TaitoZFrame, NULL, TaitoZScan,
	NULL, 320, 240, 4, 3
};
...
struct BurnDriver BurnDrvScin = {
	"scin", "sci", NULL, "1991",
	"Super Special Criminal Investigation (Negro Torino hack)\0", NULL, "Negro", "Taito-Z",
	NULL, NULL, NULL, NULL,
	BDF_GAME_WORKING | BDF_CLONE | BDF_BOOTLEG, 2, HARDWARE_TAITO_TAITOZ,
	NULL, ScinRomInfo, ScinRomName, SciInputInfo, SciDIPInfo,
	SciInit, TaitoZExit, TaitoZFrame, NULL, TaitoZScan,
	NULL, 320, 240, 4, 3
};
