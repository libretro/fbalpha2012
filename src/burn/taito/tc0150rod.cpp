...
void TC0150RODDraw(int yOffs, int pOffs, int Type, int RoadTrans, int LowPriority, int HighPriority)
{
	...
	int RoadCtrl = swapWord(RoadRam[0xfff]);
	...
	do {
		...
		RoadAClipR = swapWord(RoadRam[RoadRamIndex + 0]);
		RoadAClipL = swapWord(RoadRam[RoadRamIndex + 1]);
		RoadABodyCtrl = swapWord(RoadRam[RoadRamIndex + 2]);
		RoadBClipR = swapWord(RoadRam[RoadRam2Index + 0]);
		RoadBClipL = swapWord(RoadRam[RoadRam2Index + 1]);
		RoadBBodyCtrl = swapWord(RoadRam[RoadRam2Index + 2]);
		...
		ColBank  = (swapWord(RoadRam[RoadRamIndex + 3]) & 0xf000) >> 10;
		RoadGfxTileNum = swapWord(RoadRam[RoadRamIndex + 3]) & 0x3ff;
		RightOver = 0;
		LeftOver = 0;
		...
		if ((LineNeedsDrawing) && (Begin < End)) {
			for (i = Begin; i < End; i++) {
				if (RoadGfxTileNum) {
					GfxWord = swapWord(RoadGfx[(RoadGfxTileNum << 8) + (xIndex >> 3)]);
		...
		ColBank  = (swapWord(RoadRam[RoadRam2Index + 3]) & 0xf000) >> 10;
		RoadGfxTileNum = swapWord(RoadRam[RoadRam2Index + 3]) & 0x3ff;
		RightOver = 0;
		LeftOver = 0;
