...
static void TC0480SCPRenderLayer01(int Layer, int Opaque, unsigned char *pSrc)
{
	...
	XMemSet(pTC0480SCPTempDraw, 0, 512 * 512 * sizeof(UINT16));
	for (my = 0; my < 32; my++) {
		for (mx = 0; mx < 32; mx++) {
			Offset = 2 * TileIndex;
			Attr = swapWord(VideoRam[Offset]);
			Code = swapWord(VideoRam[Offset + 1]) & (TC0480SCPTilesNum - 1);
			Colour = Attr & 0xff;
			Flip = (Attr & 0xc000) >> 14;
			xFlip = (Flip >> 0) & 0x01;
			yFlip = (Flip >> 1) & 0x01;
	...
	do {
		...
		xIndex = sx - ((swapWord(ScrollRam[RowIndex]) << 16)) - ((swapWord(ScrollRam[RowIndex + 0x800]) << 8) & 0xffff);
...
static void TC0480SCPRenderLayer23(int Layer, int Opaque, unsigned char *pSrc)
{
	...
	do {
		ySrcIndex = ((yIndex >> 16) + swapWord(ColumnRam[(y - TC0480SCPYOffset) & 0x1ff])) &0x1ff;

		RowIndex = ySrcIndex;

		if (TC0480SCPPriReg & (Layer - 1)) {
			RowZoom = swapWord(RowZoomRam[RowIndex]);
		} else {
			RowZoom = 0;
		}
...
void TC0480SCPReset()
{
	XMemSet(TC0480SCPChars, 0, 256 * 8 * 8);
	XMemSet(TC0480SCPCtrl, 0, 0x18);
...
void TC0480SCPInit(int nNumTiles, int Pixels, int xOffset, int yOffset, int xTextOffset, int yTextOffset, int VisYOffset)
{
	...
	XMemSet(TC0480SCPRam, 0, 0x10000);
