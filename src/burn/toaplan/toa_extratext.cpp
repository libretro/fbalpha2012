...
int ToaExtraTextLayer()
{
	...
	y = 0;
	do {
		nLine = swapWord(((unsigned short*)ExtraTSelect)[y]);
		nOffset = swapWord(((unsigned short*)ExtraTScroll)[y]);

		if (y < 233) {
			for (i = 1; i < 8 && swapWord(((unsigned short*)ExtraTSelect)[y + i]) == (nLine + i) && swapWord(((unsigned short*)ExtraTScroll)[y + i]) == nOffset; i++) { }
			...
			if (i == 8) {
				...
				for (x = 0, pTile = pCurrentRow - nOffset * nBurnColumn; x < 41; x++, pTile += nTileLeft) {
					unsigned int nTile = swapWord(pTileRow[(x + nStartX) & 0x3F]);
					if (nTile && nTile != 0x20) {
