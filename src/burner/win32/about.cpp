...
#else
			TCHAR szBuild[MAX_PATH] = _T("");

			RECT rect = pdis->rcItem;
			rect.top += 1;

			_stprintf(szBuild, _T("built on ") _T(MAKE_STRING(BUILD_DATE))  _T(" (") _T(MAKE_STRING(BUILD_COMP)) _T(", ") _T(MAKE_STRING(BUILD_CPU)) _T(", ") _T(MAKE_STRING(BUILD_CPU_FP)) _T(")"));
...
static INT_PTR CALLBACK AboutProc(HWND hDlg ,UINT Msg, WPARAM wParam, LPARAM lParam)
{
	if (Msg == WM_INITDIALOG) {
		...
		HBITMAP hBitmap = (HBITMAP)LoadImage(hAppInst, _T("res\\about.bmp"), IMAGE_BITMAP, 512, 128, LR_LOADFROMFILE);
		if (!hBitmap) {
...
{
	FBADialogBox(IDD_ABOUT, parent, (DLGPROC)AboutProc);
	return 0;
}
...
//		SendDlgItemMessage(hDlg, IDC_LICENSE, EM_SETTYPOGRAPHYOPTIONS, TO_ADVANCEDTYPOGRAPHY, TO_ADVANCEDTYPOGRAPHY);
		...
		_stprintf(szWarningString, FBALoadStringEx(IDS_FIRSTRUN1), _T(APP_TITLE), szAppBurnVer);
		_tcscat(szWarningString, FBALoadStringEx(IDS_FIRSTRUN2));
 #if VER_ALPHA > 0 && (VER_ALPHA < 90 || VER_BETA < 99)
		_tcscat(szWarningString, FBALoadStringEx(IDS_FIRSTRUN3A));
 #elif VER_BETA > 0 && VER_BETA < 99
		_tcscat(szWarningString, FBALoadStringEx(IDS_FIRSTRUN3B));
 #endif
...
int FirstUsageCreate(HWND parent)
{
	FBADialogBox(IDD_FIRST, parent, (DLGPROC)FirstProc);
	return 0;
}
