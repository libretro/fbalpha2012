...
static void InitFeedback(HWND hWindow)
{
	...
	SendMessage(hWindow, WM_SETTEXT, 0, (LPARAM)szBuffer);
}
...
static int AddFeedback(HWND hWindow, TCHAR* pszText)
{
	...
	SendMessageA(hWindow, EM_SETTEXTEX, (WPARAM)&settext, (LPARAM)WtoA(pszText));
...
static int SetupDisass(int nArea)
{
	...
	SendDlgItemMessage(hDbgDlg, IDC_DBG_MAINWN1_DRP1, CB_SETCURSEL, (WPARAM)nDisassArea, 0);

	SendDlgItemMessage(hDbgDlg, IDC_DBG_MAINWN1_DRP2, CB_RESETCONTENT, 0, 0);
	if (nArea) {
		SendDlgItemMessage(hDbgDlg, IDC_DBG_MAINWN1_DRP2, CB_ADDSTRING, 0, (LPARAM)_T("little-endian 8-bit / big-endian"));
		SendDlgItemMessage(hDbgDlg, IDC_DBG_MAINWN1_DRP2, CB_ADDSTRING, 0, (LPARAM)_T("little-endian 16-bit"));
		SendDlgItemMessage(hDbgDlg, IDC_DBG_MAINWN1_DRP2, CB_ADDSTRING, 0, (LPARAM)_T("little-endian 32-bit"));
	} else {
		SendDlgItemMessage(hDbgDlg, IDC_DBG_MAINWN1_DRP2, CB_ADDSTRING, 0, (LPARAM)_T("Auto"));
	}
	SendDlgItemMessage(hDbgDlg, IDC_DBG_MAINWN1_DRP2, CB_SETCURSEL, 0, 0);
...
static int SetupMemdump(int nArea)
{
	...
	SendDlgItemMessage(hDbgDlg, IDC_DBG_MAINWN2_DRP1, CB_SETCURSEL, (WPARAM)nMemdumpArea, 0);

	SendDlgItemMessage(hDbgDlg, IDC_DBG_MAINWN2_DRP2, CB_RESETCONTENT, 0, 0);
	if (nArea) {
		SendDlgItemMessage(hDbgDlg, IDC_DBG_MAINWN2_DRP2, CB_ADDSTRING, 0, (LPARAM)_T("little-endian 8-bit / big-endian"));
		SendDlgItemMessage(hDbgDlg, IDC_DBG_MAINWN2_DRP2, CB_ADDSTRING, 0, (LPARAM)_T("little-endian 16-bit"));
		SendDlgItemMessage(hDbgDlg, IDC_DBG_MAINWN2_DRP2, CB_ADDSTRING, 0, (LPARAM)_T("little-endian 32-bit"));
	} else {
		SendDlgItemMessage(hDbgDlg, IDC_DBG_MAINWN2_DRP2, CB_ADDSTRING, 0, (LPARAM)_T("Auto"));
	}
	SendDlgItemMessage(hDbgDlg, IDC_DBG_MAINWN2_DRP2, CB_SETCURSEL, 0, 0);
...
static INT_PTR CALLBACK DebuggerDialogProc(HWND hDlg, UINT Msg, WPARAM wParam, LPARAM lParam)
{
	if (Msg == WM_INITDIALOG) {
		...
		SendDlgItemMessage(hDlg, IDC_DBG_COMMAND, WM_SETFONT, (WPARAM)hCommandFont, 0);
		...
		SendDlgItemMessage(hDlg, IDC_DBG_MAINWN1_DRP1, CB_SETCURSEL, 0, 0);
	...
	if (Msg == WM_COMMAND) {
		...
		if (LOWORD(wParam) == IDOK) {
			...
			SendDlgItemMessage(hDlg, IDC_DBG_COMMAND, WM_SETTEXT, 0, (LPARAM)_T(""));
		...
		if (HIWORD(wParam) == CBN_SELENDOK) {
			if (LOWORD(wParam) == IDC_DBG_MAINWN1_DRP1) {
				SetupDisass(SendMessage((HWND)lParam, CB_GETCURSEL, 0, 0));
			}
			if (LOWORD(wParam) == IDC_DBG_MAINWN1_DRP2) {
				nDisassAttrib = SendMessage((HWND)lParam, CB_GETCURSEL, 0, 0);
				CreateDisass(GetDlgItem(hDlg, nDisassWindow), nDisassAddress[0], nDisassCPUType);
			}
			if (LOWORD(wParam) == IDC_DBG_MAINWN2_DRP1) {
				SetupMemdump(SendMessage((HWND)lParam, CB_GETCURSEL, 0, 0));
			}
			if (LOWORD(wParam) == IDC_DBG_MAINWN2_DRP2) {
				nMemdumpAttrib = SendMessage((HWND)lParam, CB_GETCURSEL, 0, 0);
...
//			}
		...
		if (nScrollLines && nWindow) {
			...
			if (nWindow == nDisassWindow) {
				...
				if (nScrollLines < 0) {
					int nLine = SendDlgItemMessage(hDlg, nWindow, EM_LINEFROMCHAR, (WPARAM)((dwStart != (DWORD)-1) ? dwStart : dwEnd), 0);
				...
				if (nScrollLines > 0) {
					int nLine = SendDlgItemMessage(hDlg, nWindow, EM_LINEFROMCHAR, (WPARAM)dwEnd, 0);
					...
					if (nScrollLines & 1) {
						...
						if (!bAffectCaret || nScrollLines > 2 || SendDlgItemMessage(hDlg, nWindow, EM_LINEFROMCHAR, (WPARAM)dwEnd, 0) >= (nPageSize - 1)) {
			...
			if (nWindow == nMemdumpWindow) {
				...
				if (nScrollLines < 0) {
					if (nScrollLines & 1) {
						...
						if (!bAffectCaret || nScrollLines < -2 || SendDlgItemMessage(hDlg, nWindow, EM_LINEFROMCHAR, (WPARAM)dwStart, 0) == 0) {
...
int DebugExit()
{
	...
	dialogDelete(bLargeWindow ? IDD_DEBUG_LRG : IDD_DEBUG_SML);
...
int DebugCreate()
{
	...
	hDbgDlg = FBACreateDialog(bLargeWindow ? IDD_DEBUG_LRG : IDD_DEBUG_SML, hScrnWnd, (DLGPROC)DebuggerDialogProc);
	if (hDbgDlg == NULL) {
		return 1;
	}
	dialogAdd(bLargeWindow ? IDD_DEBUG_LRG : IDD_DEBUG_SML, hDbgDlg);
