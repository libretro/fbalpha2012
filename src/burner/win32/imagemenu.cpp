...
#define MENUACCOFFSET           12
#define MENUTITLEHEIGHT         10
...
vector<IMAGEMENUITEM> imageItems;
vector<IMMENUPROPS> menuProps;
...
MENUSTYLE styleProps[] = {
    ...
    // Visual Studio Extra (by CaptainCPS-X)
	{
        { 0, GetSysColor(COLOR_BTNFACE), 0, FALSE,FALSE,FALSE },
        { RGB(255,189,105), RGB(255,245,204), RGB(255,223,132),TRUE,TRUE,TRUE },
        { RGB(124,124,148), RGB(232,233,241), RGB(186,185,205),TRUE,TRUE,TRUE },
        { 0, RGB(255,255,255), RGB(196,195,172), FALSE,TRUE,FALSE }, //...............normal item props (gray)
        //{ 0, RGB(255,255,255), RGB(255,163,74), FALSE,TRUE,FALSE }, //................normal item props (FBA theme orange)
        //{ 0, RGB(255,255,255), RGB(51,93,171), FALSE,TRUE,FALSE }, //................normal item props (FBA theme II Blue)
        //{ 0, GetSysColor(COLOR_MENU), 0, FALSE,FALSE,FALSE },
        ...
        //{ RGB(255,171,63), RGB(255,192,111), RGB(255,192,111),FALSE,TRUE,TRUE }, //...checkmark props
        //{ RGB(49,106,197), RGB(255,230,232), RGB(255,230,232),TRUE,TRUE,TRUE }, //...checkmark props
        //{ RGB(251,140,60), RGB(254,128,62), RGB(254,128,62), TRUE,TRUE,FALSE },
        //{ RGB(75,75,111), RGB(49,106,197), RGB(49,106,197), TRUE,TRUE,FALSE },
...
{
    size_t tabPos = menuCaption.find_first_of( _T('\t') );
...
{
    size_t tabPos = menuCaption.find_last_of( _T('\t') );
    if (tabPos != tstring::npos)
...
void ImageMenu_OnDrawItem(WPARAM wParam, LPARAM lParam, int i)
{
    ...
    else
    {
        ...
        if (lpdi->itemState & ODS_GRAYED)
        {
            ...
            rcOffSet.left -= MENUACCOFFSET; rcOffSet.right -= MENUACCOFFSET;
        ...
        rcText.left -= MENUACCOFFSET; rcText.right -= MENUACCOFFSET;
...
LRESULT CALLBACK ImageMenu_ParentProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    static vector<HMENU> menuPopups;
...
{
    if (itemMenu == NULL || itemParentWnd == NULL)
		return FALSE;
...
BOOL ImageMenu_Create(HWND hwnd, HMENU menuHandle, BOOL isMenuBar)
{
    ...
    if (originalProc != ImageMenu_ParentProc)
    {
        ...
        //after i call SetWindowLongPtr, probably because they ARE the same
        ...
        if (!GetProp(hwnd, ORIGINALPROCPROP))
            if (SetProp(hwnd, ORIGINALPROCPROP, (HANDLE)originalProc))
                SetWindowLongPtr(hwnd, GWLP_WNDPROC, (LONG_PTR)ImageMenu_ParentProc);
    }
