...
static int InpcInit()
{
	...
	_sntprintf(szNewTitle, sizearray(szNewTitle), _T(APP_TITLE) _T(SEPERATOR_1) _T("%s %hs"), szOldTitle, bii.szName);
...
int InpcCreate()
{
	...
	hInpcDlg = FBACreateDialog(IDD_INPC, dialogGet(IDD_INPD), (DLGPROC)InpcDialogProc);
	if (hInpcDlg == NULL) {
		return 1;
	}
	dialogAdd(IDD_INPC, hInpcDlg);

	wndInMid(hInpcDlg, dialogGet(IDD_INPD));
