...
int mediaInit()
{
	...
	if (!bAudOkay) {
		...
		if (!bAudOkay) {
			...
			SplashDestroy(1);
			FBAPopupAddText(PUF_TEXT_DEFAULT, MAKEINTRESOURCE(IDS_ERR_UI_MODULE), audio.getName());
	...
	if (!bVidOkay) {
		...
		if (!bVidOkay && (bDrvOkay || bVidUsePlaceholder)) {
			// Make sure the error will be visible
			SplashDestroy(1);
			FBAPopupAddText(PUF_TEXT_DEFAULT, MAKEINTRESOURCE(IDS_ERR_UI_MODULE), VidGetName());
			FBAPopupDisplay(PUF_TYPE_ERROR);
		}
