...
static int MemcardMakeOfn()
{
	memset(&ofn, 0, sizeof(ofn));
	ofn.lStructSize = sizeof(ofn);
	ofn.hwndOwner = hScrnWnd;
	ofn.lpstrFile = szMemoryCardFile;
	ofn.nMaxFile = sizeof(szMemoryCardFile) / sizeof(TCHAR);
...
{
	TCHAR szFilter[1024];
	_stprintf(szFilter, FBALoadStringEx(IDS_DISK_FILE_CARD), _T(APP_TITLE));
	memcpy(szFilter + _tcslen(szFilter), _T(" (*.fc)\0*.fc\0\0"), 14 * sizeof(TCHAR));

	_stprintf (szMemoryCardFile, _T("memorycard"));
	MemcardMakeOfn();
	ofn.lpstrTitle = FBALoadStringEx(IDS_MEMCARD_CREATE);
...
int	MemCardSelect()
{
	...
	pszTemp += _stprintf(pszTemp, FBALoadStringEx(IDS_DISK_ALL_CARD));
	memcpy(pszTemp, _T(" (*.fc, MEMCARD.\?\?\?)\0*.fc;MEMCARD.\?\?\?\0"), 38 * sizeof(TCHAR));
	pszTemp += 38;
	pszTemp += _stprintf(pszTemp, FBALoadStringEx(IDS_DISK_FILE_CARD), _T(APP_TITLE));
	memcpy(pszTemp, _T(" (*.fc)\0*.fc\0"), 13 * sizeof(TCHAR));
	pszTemp += 13;
	pszTemp += _stprintf(pszTemp, FBALoadStringEx(IDS_DISK_FILE_CARD), _T("MAME"));
	memcpy(pszTemp, _T(" (MEMCARD.\?\?\?)\0MEMCARD.\?\?\?\0\0"), 28 * sizeof(TCHAR));

	MemcardMakeOfn();
	ofn.lpstrTitle = FBALoadStringEx(IDS_MEMCARD_SELECT);
	ofn.lpstrFilter = szFilter;
