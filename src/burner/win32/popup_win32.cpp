		...
		case WM_INITDIALOG: {
			...
			switch (nPopupFlags & 7) {
				case PUF_TYPE_ERROR:
					...
					nBeep = MB_ICONHAND;
					pszTypeEnglish = FBALoadStringEx(IDS_ERR_ERROR, false);
					pszTypeLocal = FBALoadStringEx(IDS_ERR_ERROR);
					break;
				case PUF_TYPE_WARNING:
					nIcon = OIC_WARNING;
					nBeep = MB_ICONEXCLAMATION;
					pszTypeEnglish = FBALoadStringEx(IDS_ERR_WARNING, false);
					pszTypeLocal = FBALoadStringEx(IDS_ERR_WARNING);
					break;
				default:
					nIcon = OIC_INFORMATION;
					nBeep = MB_ICONASTERISK;
					pszTypeEnglish = FBALoadStringEx(IDS_ERR_INFORMATION, false);
					pszTypeLocal = FBALoadStringEx(IDS_ERR_INFORMATION);
					break;
			}
			...
			// Set the caption
			_sntprintf(szCaption, sizearray(szCaption), _T(APP_TITLE) _T(" %s"), pszTypeLocal);
...
int FBAPopupAddText(int nFlags, TCHAR* pszFormat, ...)
{
	...
	if (IS_INTRESOURCE(pszFormat)) {
		pszStringEnglish = FBALoadStringEx((UINT)pszFormat, false);
	} else {
	...
	if (nFlags & PUF_TEXT_TRANSLATE) {
		...
		if (IS_INTRESOURCE(pszFormat)) {
			pszStringLocal = FBALoadStringEx((UINT)pszFormat);
		} else {
...
int FBAPopupDisplay(int nFlags)
{
	...
	if (!(nPopupFlags & PUF_TYPE_LOGONLY)) {
		FBADialogBox(IDD_POPUP, hSelDlg ? hSelDlg : hScrnWnd, (DLGPROC)FBAPopupProc);
	}
