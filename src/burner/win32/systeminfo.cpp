		...
		if (osvi.dwPlatformId == VER_PLATFORM_WIN32_NT) {
			if (osvi.dwMajorVersion == 5 && osvi.dwMinorVersion == 0) {
				...
				if (osvi.wProductType == VER_NT_WORKSTATION
					...
					AddText(_T("Microsoft Windows XP Professional x64 Edition "));
				}
//				else if (GetSystemMetrics(SM_SERVERR2)) {
//					AddText(_T("Microsoft Windows Server 2003 \"R2\" "));
//				}
				else
					AddText(_T("Microsoft Windows Server 2003 "));
			}
			else if (osvi.dwMajorVersion == 6 && osvi.dwMinorVersion == 0) {
				 if (osvi.wProductType == VER_NT_WORKSTATION )
					AddText(_T("Windows Vista "));
				 else
				 	AddText(_T("Windows Server \"Longhorn\" "));
			}
			else if (osvi.dwMajorVersion == 6 && osvi.dwMinorVersion == 1) {
				if (si.wProcessorArchitecture == PROCESSOR_ARCHITECTURE_AMD64) {
					AddText(_T("Windows 7 x64 "));
				} else {
		...
		if (osvi.dwPlatformId == VER_PLATFORM_WIN32_WINDOWS) {
			...
			if (osvi.dwMajorVersion == 4 && osvi.dwMinorVersion == 10) {
				...
				if (osvi.szCSDVersion[1] == _T('A') || osvi.szCSDVersion[1] == _T('B')) {
					AddText(_T(" SE"));
				}
			}
...
int PrintCPUInfo()
{
...
#if 0
	// get cpu info, rewritten by regret
	char CPUString[0x20];
	...
	*((int*)(CPUString + 8)) = CPUInfo[2];
	...
	if (nExIds >= 0x80000004) {
		szModel = AtoW(CPUBrandString);
	}

	// Finally, print out the info
	if (szModel) {
		AddLine(_T("%12hs, %s"), CPUString, szModel);
	} else {
		AddLine(_T("%12hs, Unknown CPU"), CPUString);
	}
#endif
...
int PrintGlobalMemoryInfo()
{
	...
	HMODULE hPsapiDLL = LoadLibrary(_T("psapi.dll"));
	if (hPsapiDLL) {
		...
		if (pGetProcessMemoryInfo) {
			...
			if (pGetProcessMemoryInfo(GetCurrentProcess(), &pmc, sizeof(pmc))) {
				...
				size_t length = _tcslen(_T(APP_TITLE));
				if (length > 12) {
					length = 12;
				}
...
{
	TCHAR szName[1024] = _T("");  DWORD nNameSize = sizearray(szName);
	TCHAR szClass[1024] = _T(""); DWORD nClassSize = sizearray(szClass);
	...
	for (int i = 0; !bFound && RegEnumKeyEx(hKey, i, szName, &nNameSize, NULL, szClass, &nClassSize, &ftLastWriteTime) != ERROR_NO_MORE_ITEMS; i++) {
		TCHAR szSubName[1024] = _T("");  DWORD nSubNameSize = sizearray(szSubName);
		TCHAR szSubClass[1024] = _T(""); DWORD nSubClassSize = sizearray(szSubClass);
		...
		for (int j = 0; !bFound && RegEnumKeyEx(hSubKey, j, szSubName, &nSubNameSize, NULL, szSubClass, &nSubClassSize, &ftSubLastWriteTime) != ERROR_NO_MORE_ITEMS; j++) {
			...
			TCHAR szKeyValue[1024]; DWORD nSize = sizearray(szKeyValue);
			DWORD nType;
			...
			nType = REG_SZ; nSize = sizearray(szKeyValue);
			...
			for (int i = 0; pEnumDisplayDevices(NULL, i, &ddAdapter, 0); i++) {
				...
				if (!(ddAdapter.StateFlags & DISPLAY_DEVICE_MIRRORING_DRIVER)) {
					for (int j = 0; pEnumDisplayDevices(ddAdapter.DeviceName, j, &ddDisplay, 0); j++) {
						...
						TCHAR szMfg[1024] = _T("");		   DWORD nMfgSize = sizearray(szMfg);
						TCHAR szDeviceDesc[1024] = _T(""); DWORD nDeviceDescSize = sizearray(szDeviceDesc);
						...
						if (ddDisplay.StateFlags & DISPLAY_DEVICE_ACTIVE) {
							if (!_tcsnicmp(ddDisplay.DeviceKey, _T("\\Registry\\Machine\\"), 18)) {
								TCHAR szDriver[1024]; DWORD nSize = sizearray(szDriver);
								DWORD nType = REG_SZ;
...
#else
	AddLine(_T("MAME Z80 emulation core enabled for Z80 emulation."));
#endif
	AddLine(_T(""));
	...
	switch (Msg) {
		case WM_INITDIALOG: {
			...
			if (pExceptionPointers) {
				...
				_stprintf(szText, _T("Fatal exception #%08X.%08X!"), pExceptionPointers->ExceptionRecord->ExceptionCode, (unsigned int)pExceptionPointers->ExceptionRecord->ExceptionAddress);
			...
			if (HIWORD(wParam) == BN_CLICKED) {
				...
				if (LOWORD(wParam) == IDC_SYSINFO_LOG) {
					...
					if (pExceptionPointers) {
						...
						_stprintf(szFilter, FBALoadStringEx(IDS_DISK_TEXT));
						...
						ofn.lpstrTitle = FBALoadStringEx(IDS_DISK_SAVEREPORT);
...
LONG CALLBACK ExceptionFilter(_EXCEPTION_POINTERS* pExceptionInfo)
{
	...
	nRet = FBADialogBox(IDD_EXCEPTION, hScrnWnd, (DLGPROC)SysInfoProc);
...
int SystemInfoCreate(HWND parent)
{
	...
	FBADialogBox(IDD_SYSINFO, parent, (DLGPROC)SysInfoProc);
	return 0;
}
