...
#define PAGE_SIZE	0x00000800 // 400 would be better...
...
void Arm7Init(int num) // only one cpu supported
{
	for (int i = 0; i < 3; i++) {
#if defined (_XBOX)
		membase[i] = (unsigned char**)VirtualAlloc(NULL, PAGE_COUNT *sizeof(int),MEM_COMMIT|MEM_RESERVE|MEM_LARGE_PAGES, PAGE_READWRITE); 
#elif defined (SN_TARGET_PS3)
		sys_addr_t addr;
		sys_memory_allocate(PAGE_COUNT * sizeof(int),SYS_MEMORY_PAGE_SIZE_1M,&addr);
		membase[i] = (unsigned char **)addr;
#else
...
void Arm7Exit() // only one cpu supported
{
	for (int i = 0; i < 3; i++) {
		if (membase[i]) {
#if defined (_XBOX)
			VirtualFree(membase[i],0, MEM_DECOMMIT|MEM_RELEASE);
#elif defined (SN_TARGET_PS3)
		    sys_memory_free((sys_addr_t)membase[i]); 
#else
			free (membase[i]);
#endif
			membase[i] = NULL;
...
void Arm7_program_write_word_32le(unsigned int addr, unsigned short data)
{
	...
	if (membase[WRITE][addr >> PAGE_SHIFT] != NULL) {
		*((unsigned short*)(membase[WRITE][addr >> PAGE_SHIFT] + (addr & PAGE_WORD_AND))) =  swapWord(data);
		return;
	}
...
void Arm7_program_write_dword_32le(unsigned int addr, unsigned int data)
{
	...
	if (membase[WRITE][addr >> PAGE_SHIFT] != NULL) {
		*((unsigned int*)(membase[WRITE][addr >> PAGE_SHIFT] + (addr & PAGE_LONG_AND))) =  swapLong(data);
		return;
	}
...
unsigned short Arm7_program_read_word_32le(unsigned int addr)
{
	...
	if (membase[ READ][addr >> PAGE_SHIFT] != NULL) {
		return  swapWord(*((unsigned short*)(membase[ READ][addr >> PAGE_SHIFT] + (addr & PAGE_WORD_AND))));
	}
...
unsigned int Arm7_program_read_dword_32le(unsigned int addr)
{
	...
	if (membase[ READ][addr >> PAGE_SHIFT] != NULL) {
		return swapLong(*((unsigned int*)(membase[ READ][addr >> PAGE_SHIFT] + (addr & PAGE_LONG_AND))));
	}
...
unsigned short Arm7_program_opcode_word_32le(unsigned int addr)
{
	...
	if (membase[FETCH][addr >> PAGE_SHIFT] != NULL) {
		return swapWord(*((unsigned short*)(membase[FETCH][addr >> PAGE_SHIFT] + (addr & PAGE_WORD_AND))));
	}
...
unsigned int Arm7_program_opcode_dword_32le(unsigned int addr)
{
	...
	if (membase[FETCH][addr >> PAGE_SHIFT] != NULL) {
		return swapLong(*((unsigned int*)(membase[FETCH][addr >> PAGE_SHIFT] + (addr & PAGE_LONG_AND))));
	}
